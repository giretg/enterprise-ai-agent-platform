...
async function main() {
  ...
  await check('cache: csupa null nem mintázat, 0-ás értékek mellett viszont az', () => {
    ...
    assert.equal(
      missing.patterns.some((p) => p.kind === 'cache_prefix_break' && p.savingsTokens !== null),
      false,
    )
    assert.ok(describeEfficiencyCacheDataStatus('missing').includes('nem ad cache-adatot'))
