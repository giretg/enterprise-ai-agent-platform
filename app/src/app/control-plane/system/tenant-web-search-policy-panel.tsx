...
type PolicyForm = {
  provider: 'stub' | 'custom_search_api' | 'platform_hosted_search'
...
const PROVIDER_HELP: Record<PolicyForm['provider'], { title: string; text: string }> = {
  stub: {
    title: 'Stub: tesztmód',
    text: 'Determinisztikus teszteredmények, nincs valódi internetes keresés.',
  },
  platform_hosted_search: {
    title: 'Platform-hosted search',
    text: 'A tenant policy (allowlist, limitek) itt él; a kereső API kulcsot a platform admin állítja be.',
  },
  custom_search_api: {
    title: 'Saját kereső API',
    text: 'Tenant saját endpoint + API kulcs (secret-ref). Költség tenant-specifikus.',
  },
}
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <label className="space-y-1 text-xs font-medium text-ink-soft">
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateForm('provider', event.target.value as PolicyForm['provider'])
                  }
                ...
                >
                  <option value="platform_hosted_search">platform_hosted_search</option>
