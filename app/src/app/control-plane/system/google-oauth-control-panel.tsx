...
import { upsertPlatformGoogleOAuth } from '@/app/actions/connector-grants'

export type PlatformGoogleOAuthView = {
  configured: boolean
  persisted: boolean
...
function sourceLabel(source: PlatformGoogleOAuthView['source']): string {
  ...
  if (source === 'env') return 'környezeti változó (GMAIL_OAUTH_*)'
  if (source === 'tenant_legacy') return 'korábbi tenant-beállítás'
  return 'nincs forrás'
}

export function GoogleOAuthControlPanel({
  initial,
  canEdit,
}: {
  ...
  function save() {
    ...
    startTransition(async () => {
      const res = await upsertPlatformGoogleOAuth({
      ...
      if (res.success) {
        ...
        setClientSecret('')
        setEditing(false)
        setMessage({ tone: 'ok', text: 'Platform Google OAuth beállítás mentve.' })
      } else {
    ...
    <Card title="Google OAuth alkalmazás">
      <div className="space-y-4">
        <p className="text-sm text-ink-soft">
          Ez az Enterprise AI Agent Google Cloud OAuth clientje — nem a tenantot azonosítja.
          Minden tenant Google connectorai ezt használják; a felhasználók utána a saját
          fiókjukkal lépnek be.
        </p>
        ...
        {view.configured ? (
          ...
          <p className="text-sm text-ink-soft">Még nincs platform-szintű Google OAuth client.</p>
        )}
