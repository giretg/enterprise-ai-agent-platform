...
const trustKeys = ['trustApproval', 'trustAudit', 'trustBudget', 'trustGdpr'] as const
...
export default async function Home({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const chat = [
    { from: 'user' as const, who: t('chatUser'), text: t('chatUserText') },
    { from: 'agent' as const, who: t('chatAgent'), text: t('chatAgentText') },
  ]
  const steps = [
          ...
          <div className="atelier-card animate-rise overflow-hidden [animation-delay:120ms]">
            <div className="flex items-center justify-between border-b border-line bg-card-2/60 px-5 py-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-ink">{t('chatName')}</p>
                  <p className="text-[11px] text-ink-faint">{t('chatRole')}</p>
                </div>
              </div>
            ...
            <div className="space-y-4 px-4 py-5 sm:px-5">
              {chat.map((m, i) => (
                <div key={i} className={`flex items-end gap-2.5 ${m.from === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`max-w-[78%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed shadow-sm ${m.from === 'user' ? 'rounded-tr-md bg-coral text-card' : 'rounded-tl-md border border-line bg-card text-ink-soft'}`}>
                    {m.text}
                    {m.from === 'agent' && (
                      <p className="mt-2 border-l-2 border-line pl-2.5 text-xs italic leading-relaxed text-ink-faint">{t('chatDraftText')}</p>
                    )}
                  </div>
                </div>
              ))}
              <div className="mx-auto w-[92%] rounded-lg border border-honey/40 bg-honey/10 p-3.5">
                ...
                <dl className="mt-3 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
                  <dt className="text-ink-faint">{t('approvalTo')}</dt>
                  <dd className="text-ink">{t('approvalToValue')}</dd>
                  <dt className="text-ink-faint">{t('approvalSubject')}</dt>
                  <dd className="text-ink">{t('approvalSubjectValue')}</dd>
                  <dt className="text-ink-faint">{t('approvalSource')}</dt>
                  <dd className="text-ink">{t('approvalSourceValue')}</dd>
                </dl>
                <div className="mt-3 flex gap-2">
                  <span className="rounded-full bg-sage/20 px-4 py-1.5 text-[11px] font-semibold text-sage">{t('approvalApprove')}</span>
                  <span className="rounded-full bg-card px-4 py-1.5 text-[11px] font-semibold text-ink-faint">{t('approvalRevise')}</span>
                </div>
              </div>
            </div>
          </div>
