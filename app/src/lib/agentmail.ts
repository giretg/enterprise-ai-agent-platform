/**
 * AgentMail (EU) admin kliens: a tenant szervezeti kulcsával postafiókot listáz/hoz létre,
...
import { AGENTMAIL_API_BASE } from '@/domain/connector-template/builtin-templates'
...
const INBOX_BASE_PREFIX = `${AGENTMAIL_API_BASE}/inboxes/`
...
export function inboxIdFromConnectorBaseUrl(baseUrl: unknown): string | null {
  if (typeof baseUrl !== 'string' || !baseUrl.startsWith(INBOX_BASE_PREFIX)) return null
  const segment = baseUrl.slice(INBOX_BASE_PREFIX.length).replace(/\/+$/, '')
...
export async function createAgentMailInbox(
  ...
  const raw = await call<RawInbox>(apiKey, 'POST', '/inboxes', {
