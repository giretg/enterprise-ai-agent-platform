...
5. **A képességlista nem azonos a hívható MCP toolokkal.** A 2026-09-23-as próbán a publikált definícióban `gmail_send` és `gmail_create_draft` engedélyezett volt, de a 34 MCP tool között nem szerepeltek. Ez a Gmail írási toolok (#637) óta már nem áll: küldés, piszkozat, címkézés és kuka MCP-n hívható. A `kb_get_document` továbbra is hívható anélkül, hogy külön képességként szerepelne. A PR utasítja az agentet, hogy a `tools/list` legyen a hívható toolok forrása.
