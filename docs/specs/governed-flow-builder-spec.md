...
- **D12 — Step-outcome kontraktus + implicit hiba-él (kaszkád-védelem).** Minden step a próza mellé kötelező gépi `outcome` státuszt (`ok | blocked | failed`) ad; a hard signalokat (tool-denied, tool-loop exception, hiányzó output contract) a **runtime kényszeríti rá determinisztikusan**, nem az agent optimista önbevallására bízva. A `kb_search` 0 találat csak retrieval-diagnosztika, önmagában nem blokkol. Az elágazás **implicit hiba-éllel** történik (BPMN *error boundary event* modell), NEM kötelező döntés-node-dal minden step után. `blocked`/`failed` step sosem lesz happy-path `done`, és a kimenetét nem propagálja a következő step inputjába. Részletek: §10.
...
**Cél:** megakadályozni, hogy egy tartalmilag sikertelen lépés *sikerként* propagálódjon a következő lépésbe. Kiváltó eset: egy step „Sajnálom, nem tudom elérni..." prózát írt, majd `board_write`-tal `state: 'done'`-ra állt (`general-task-runtime.ts` `buildStepCompletionPayload` → board_write done), és a downstream step ezt a bocsánatkérést **forrásként** („drafted proposal") nyelte le → a hiba végiggörgött a folyamaton.
...
**Determinista felülírás (a lényeg):** a státuszt nem az agent önbevallására bízzuk. A hard signalokat a runtime kényszeríti ki: ha a tool-broker `denied`-et ad, a tool-loop exception-nel zárul, vagy a kötelező output contract mezők hiányoznak, a runtime az `outcome`-ot `blocked`/`failed`-re állítja, függetlenül attól, mit írt az agent. A pre-fetch `kb_search` 0 találata csak retrieval-diagnosztika: önmagában nem bizonyítja, hogy a lépés nem teljesíthető más forrásból. A meglévő `buildStepCompletionPayload` / `outputRequiredFields` ezzel az `outcome` mezővel bővül, és a completion-payload validátora kötelezővé teszi.
...
- [ ] Hard signal (tool-denied, tool exception, hiányzó kötelező output mező) determinisztikusan `blocked`/`failed` outcome-ot ad, az agent optimista `done`-ját felülírva. A `kb_search` 0 találat önmagában nem hard signal.
