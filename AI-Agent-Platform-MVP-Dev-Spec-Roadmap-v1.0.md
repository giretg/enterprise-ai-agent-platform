...
| Epik 3 — Agent Registry + Model Gateway | Részben kész | `roleInstruction` és `behaviorProfile` külön mező/verzió; S2 valódi ChatGPT OAuth mediáció éles spike. *(latency/status + guardrail kész 2026-06-15; recipe snapshot kész 2026-06-15; **OpenAI-kompatibilis Gateway API + stub provider kész 2026-06-16**.)* |
...
| Epik 5 — Harness (Goose) + Dispatcher | Részben kész | Éles GCP VPC firewall proof + valódi Goose E2E stub nélkül. *(… **Cloud Run Job deploy/smoke + launcher refaktor + scenario23** kész 2026-06-16 session 12; docker-local path session 11; N4 acceptance zöld.)* |
...
1. **Éles GCP proof:** VPC firewall deny-by-default + `npm run harness:cloud-run-smoke` valódi projekten.
