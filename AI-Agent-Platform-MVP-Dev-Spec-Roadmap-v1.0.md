...
**Frissítve:** 2026-06-16 (session 15)
...
| Model Gateway provider-absztrakció | Részben kész | A Gemini-függőség ki lett vezetve az appból. A `ModelGateway` most csak `chatgpt-oauth` providert fogad, és belső `CHATGPT_OAUTH_PROVIDER_URL` / `CHATGPT_OAUTH_PROVIDER_KEY` adapteren át hív. **S2 stub kész (2026-06-16):** `CHATGPT_OAUTH_PROVIDER_URL=stub` beépített acceptance/dev stub; külön `npm run s2:stub` HTTP stub szerver. Éles App Hostingon `CHATGPT_OAUTH_STUB=true` env-vel a Gateway stub választ ad valódi OAuth nélkül — ez tette zölddé a Cloud Run éles smoke-ot (a `HARNESS_CALLBACK_TOKEN` App Hosting RUNTIME secret a callbackhez). **OpenAI-kompatibilis Gateway API kész (2026-06-16):** `POST /api/v1/gateway/v1/chat/completions` agent API-kulccsal, `X-Ticket-Id` headerrel, OpenAI chat completion formátumban — a Goose provider réteg erre köthető (`OPENAI_BASE_URL`). A valódi ChatGPT OAuth mediáció az S2 spike éles validációja.
...
| Epik 3 — Agent Registry + Model Gateway | Részben kész | Hátra: S2 valódi ChatGPT OAuth mediáció éles spike. *(latency/status + guardrail kész 2026-06-15; recipe snapshot kész 2026-06-15; **OpenAI-kompatibilis Gateway API + stub provider kész 2026-06-16**; **`roleInstruction`/`behaviorProfile` külön mező + verziózás + `updateInstruction`/`updateAgentInstruction` kész 2026-06-16 session 15**.)* |
...
2. **S2 éles OAuth:** valódi ChatGPT OAuth provider; heti kvóta/budget cap validáció.
...
7. ~~**Epik 3:** `roleInstruction` / `behaviorProfile` külön verziózás az agent registry-ben.~~ ✅ Kész (session 15) — külön mező + al-verzió + `updateInstruction` + `agent.version` audit + UI; scenario26 zöld. Hátra Epik 3-ban: csak az S2 valódi ChatGPT OAuth éles spike.
