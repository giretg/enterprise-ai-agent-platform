...
**Frissítve:** 2026-06-16 (session 3)
...
| Dispatcher előfeltételek | Részben kész | Bekerült `tickets.lock_token`, `tickets.locked_at`, ready-ticket keresés, célzott `dispatchTicket`, idempotens lock, lock release, per-agent napi call/token budget ellenőrzési pont, `dispatch.start` / `dispatch.error` / `dispatch.budget_blocked` audit és `ticket_transitions` sor a dispatcher-indulásról. Lokális MVP launcher futtatja a wiki runtime-ot ugyanazon launcher-szerződésen át. **S1 előkészítés kész (2026-06-16):** `HARNESS_LAUNCHER_MODE=cloud-run-job` esetén a dispatcher a Cloud Run v2 Jobs API-t hívja, `TICKET_ID` / `AGENT_ID` / `DISPATCH_LOCK_TOKEN` konténer env override-dal, metadata-token vagy opcionális bearer token auth-tal, és `launcherMode` / `executionName` audit metaadatokkal. Hátra: valódi Goose image/recipe, completion/timeout callback és LISTEN/NOTIFY + cron worker. |
...
| Epik 2 — IAM / RBAC | Részben kész | Hátra: agent scope-ok további szigorítása; Clerk-szerep szinkron. *(Admin meghívás+redeem flow, utolsó admin lock-out védelem, self-protection, suspended kill-switch, hozzáférési audit és negatív tesztek kész 2026-06-16; admin IAM UI + redeem képernyő kész 2026-06-16.)* |
...
| Epik 5 — Harness (Goose) + Dispatcher | Részben kész | Lokális inline wiki launcher és célzott dispatcher-indítás kész (2026-06-16): `ready` ticket lockolása, `in_progress` transition history, wiki runtime futtatás, lock release, hiba esetén `dispatch.error` és ticket visszaállítás. Cloud Run Job launcher adapter kész (2026-06-16) env-alapú váltással. Hátra: S1-S4 spike; Goose image/recipe; Cloud Run Job lokális/GCP proof; dispatcher futtató processz `LISTEN/NOTIFY` + cron safety net; job completion/timeout lock release; egress deny és developer-extension lezárás igazolása. |
...
| Epik 7 — Sandbox use case (wiki) + App Registry stretch **[CR-MVP-001]** | Részben kész | `askWiki` most `ready` ticketet hoz létre és célzott dispatcher-indítással futtatja a lokális wiki launchert; a citált válasz UI megvan. Az S2/valódi ChatGPT OAuth + Cloud Run Job teljes E2E még hátra van; GCS/GCS-helyettes absztrakció `uploadDocument`-hez; régi bookkeeper runtime kiváltása wiki runtime-mal. *(Dokumentumfeltöltés, KB-linking UI és `processDocumentForWiki` server action kész 2026-06-15; App Registry v0 A0 preview/export + N5 acceptance kész 2026-06-16; lokális dispatcher wiki path kész 2026-06-16.)* |
...
1. **S1-S4 spike folytatása:** a Cloud Run Job launcher adapter már kész; következő lépés a Goose image/recipe proof és a job-completion callback/lock-release szerződés lezárása.
2. **Wiki sandbox E2E S2-vel:** a lokális `ready` ticket + dispatcher + wiki runtime útvonal lefuttatása valódi ChatGPT OAuth providerrel, majd a Cloud Run launcherre csere.
...
6. **Deploy hardening [CR-MVP-001]:** az elkészült App Registry v0 preview route külön cookieless preview originre mozgatása, ha a stretch demó éles környezetben is kell.
