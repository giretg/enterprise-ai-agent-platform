...
- Rendben talált kontrollok: az `AllowlistAuthorizer` szerep-sablon → capability → connector-feloldás → tenant-izoláció → grant/scope láncot fut, `acting_user`-t követel user-delegált connectornál, felfüggesztett usert kizár; a `resolveActingUserId`/`resolveActingUserId`/`resolveActingTenantId` sosem fogad el a kérés törzséből választott identitást (csak a saját ticket/conversation kontextusból); a `board_write` a jegyet a hívó agenthez köti; a `gmail_send` a chat-loopban következmény-kapuzott (kártya + audit-nyom). A jóváhagyást beállító `approveGmailSend` server action `approver` szerepet és tenant-scope-ot követel.
