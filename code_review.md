...
## 2026-09-03 - Munka-projekt memóriahatár: a webes chat és a Telegram projektválasztója megkerülhette a projektkatalógust

- Áttekintett, korábban külön nem naplózott komponens: a **munka-projekt katalógus és a tartós agent-memória scope-jának bejövő útjai** — `app/src/app/api/v1/agent-chat/stream/route.ts`, `app/src/domain/channel/channel-agent-access-service.ts`, a `WorkProjectService` kioszthatósági kapuja, valamint a chat-/csatorna-forduló projektkötése. Ez dönti el, hogy egy beszélgetés mely projekt döntéseit, korlátait és korábbi munkáját kapja meg kontextusként.
- Rendben talált kontrollok: a ticket-indítás, a projektváltó server action és a projektkatalógus már tenant-szinten, aktív állapotra ellenőrizte a kulcsot; az `__general__` gyűjtő explicit, biztonságos alapérték; a memória-lekérdezés minden projektkulcsot auditálhatóan kezel.
- **Lelet (magas, memória-scope integritás / adatelkülönítés):** a webes chat streaming API a kliens által küldött `projectKey`-t közvetlenül írta az új vagy meglévő beszélgetéshez. Egy kézzel küldött kérés így nem létező vagy archivált „fantom” scope-ot hozhatott létre, illetve egy szálat másik projekt memóriájára irányíthatott. A Telegram-kötés korábbi projektkulcsai is csak formai ellenőrzést kaptak, ezért régi sor vagy archiválás után ugyanilyen érvénytelen scope kerülhetett volna a következő forduló promptjába.
- Javítás: a webes chat ugyanazt a szerveroldali `assignableKey` kaput használja, mint a ticketek; ismeretlen, archivált vagy hibás kulcs 400-as, közérthető hibával leáll. A Telegram grant létrehozása és módosítása csak aktív, saját tenantbeli projektre enged; egy régi vagy időközben archivált grantot a feloldási úton fail-closed módon az `__general__` scope-ra normalizálunk. A jóváhagyás-folytatás szándékosan nem fogad el új projektválasztást, így nem írja át a már jóváhagyott beszélgetés kontextusát.
- Üzleti hatás: a projekt nem pusztán címke, hanem meghatározza, milyen korábbi döntések és ügyadatok segítik az agent válaszát. A javítás megakadályozza, hogy egy kliensoldali kérés vagy elavult Telegram-kötés összekeverje két munka kontextusát, és így hibás vagy jogosulatlan üzleti információra épüljön a válasz.
- Ellenőrzés: `npx tsc --noEmit --pretty false`, `npx tsx scripts/work-project.test.ts`, `npx tsx scripts/channel-agent-access.test.ts`, célzott ESLint és `git diff --check` zöld. Az új regressziók lefedik az ismeretlen/archivált kulcs tiltását, illetve a korábbi vagy utólag archivált Telegram-kulcs biztonságos visszaesését az Általános scope-ra.
- PR: https://github.com/giretg/enterprise-ai-agent-platform/pull/419 (`codex`, `codex-automation`, `security` címkékkel).
