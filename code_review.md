...
## 2026-08-26 - Publikus gateway végpont (OpenAI-kompatibilis `/api/v1/gateway/v1/chat/completions`): NEM megbízható kontextus-fejlécek kiejtették a modellhívás költség-rekordját (keret- és audit-vakfolt) + cross-tenant költség-tapadás

- Áttekintett, korábban külön nem naplózott komponens: a **publikus, OpenAI-kompatibilis modell-gateway HTTP-végpont** (`app/src/app/api/v1/gateway/v1/chat/completions/route.ts`). Ezt a felületet a külső agent-futtató (goose/harness) hívja agent-API-kulccsal; a kérésből modellhívás lesz a `ModelGateway`-en át. A domain-réteget (routing, keret, privacy, sensitivity) korábbi review-k lefedték (Model Gateway budget 08-05, governed routing 07-06), de EZT a HTTP-belépőt — az általa fogadott, kliens-vezérelt kontextus-fejlécek kezelését — még nem. Ellenőrzött és rendben talált kontrollok: az auth (`authenticateAgentRequest` + `tool:invoke` scope), a tenant a `ModelGateway`-ben az **agentből** oldódik fel (nem a kliens fejlécéből), a privacy-transzformáció és a routing-kapuzott modell-override a `prepareModelCall`-ban fut, a keret-/guardrail-kapu aktív. A klienstől jövő `model` override a routing-engine-en át érvényesül, nem kerüli meg a policy-t.
- **Lelet (magas, költség-kontroll integritás + audit-vakfolt; másodlagosan cross-tenant költség-tapadás):** a végpont két kontextust NEM megbízható HTTP-fejlécből vett és nyersen továbbadott a gateway-nek: `x-agent-version` (`Number.parseInt` → **NaN**, mert a `?? agent.currentVersion` fallback a NaN-t nem fogja — csak `null`/`undefined`-ot), és `x-ticket-id` (tetszőleges string). A `ModelGateway.call` ezeket a `ModelCall` költség-rekordba írja, tipizált oszlopokba: `agent_version Int?`, `ticket_id @db.Uuid` (FK a `Ticket`-re). A lefolyás sorrendje kulcsfontosságú: a **provider-hívás (és a valós szolgáltatói költség) ELŐBB történik**, a `modelCalls.create` UTÁNA. Ezért egy rosszul formázott fejléc nem a kérés elején bukott el, hanem a rekord **perzisztálásakor**:
  - `x-agent-version: abc` → NaN egy `Int` oszlopban → az insert eldobja a rekordot;
  - `x-ticket-id: <érvényes-formájú, de nem létező UUID>` → FK-sértés → az insert eldobja a rekordot.
  Következmény: **a modellhívás megtörtént, a platform fizetett a szolgáltatónak, de a költség-rekord elveszett** — a hívás láthatatlanul kiesett a napi keret könyveléséből és az audit-nyomból. Ez pontosan a platform egyik alapígéretét (AGENTS.md: „a költségeket kontrollálhatja") sérti, ráadásul egy nyilvános, kliens-vezérelt fejléccel triggerelhető, ismételten. Másodlagosan: egy **létező, de MÁS tenant** ticketjének UUID-jét megadva a `ModelCall.ticketId` idegen tenant ticketjéhez kötötte a költség-rekordot (a per-ticket költség-nézet és guardrail-számláló így szennyeződik). A testvér tool-invoke végpont (`/api/v1/agent/tools`) ezt már helyesen zárta: a `toolInvokeSchema` a `ticketId`-t `z.string().uuid()`-ként validálja — a gateway-végpont viszont nyers fejlécet olvasott, validáció nélkül.
- Javítás (a belépő-határon, ahol a nem megbízható adat érkezik — a chat-út szerveroldali, megbízható `ticketId`-t ad, ezért érintetlen):
  - Új, tiszta, tesztelhető szűrő-modul `app/src/lib/gateway-request-context.ts`: `parseAgentVersionHeader` (csak tisztán számjegy → nemnegatív egész, egyébként `undefined` — a NaN SOHA nem csúszik át) és `parseTicketIdHeader` (csak jól formázott UUID). Az érvénytelen fejléc **fail-safe** `undefined`: a hívás lefut és rögzül, csak a hibás kontextus esik ki — nem vész el a költség-rekord.
  - A route az `x-ticket-id`-t a hívó agent szervezetéhez **köti** (`resolveTenantOwnedTicketId`): tenant-scoped agentnél csak a saját tenant létező ticketje megy át; idegen/ismeretlen ticket némán ticket-kontextus nélkül fut. A megosztott (platform, `tenantId=null`) agent a korábbi, tágabb viselkedést tartja — nincs regresszió a legit goose/harness híváson (saját tenant ticketje átmegy).
  - A hibakezelő már nem szivárogtatja a belső hibaüzenetet (pl. „Gateway guardrail: ticket &lt;uuid&gt; reached N…") a külső API-kliensnek: általános üzenet megy vissza, a részlet a szerver-logba. A státusz-választás típus szerint dönt (`e instanceof GatewayBudgetError` → `429`), NEM a hibaüzenet szövegére illesztve — így a keret-, guardrail- ÉS érzékenység-kapu elutasítása egységesen `429`, és egy átfogalmazott üzenet nem csúsztatja el a státuszt (a `/code-review` mindkét tengelye ezt a stringmatch-törékenységet jelezte).
- Üzleti hatás: egy enterprise AI platform költség-kontroll ígérete csak akkor tartható, ha MINDEN díjazott modellhívás beleszámít a keretbe és megjelenik az auditban. E hiba mellett egy hibás (vagy szándékosan manipulált) HTTP-fejléc a hívást a szolgáltatónál díjaztatta, a platform könyvelésében viszont eltüntette — a napi keret túlléphető, a költség-riport csendben alulmér. A javítás garantálja, hogy a hibás fejléc soha ne buktassa meg a rekordot, és hogy idegen tenant ticketjéhez ne tapadjon költség.
- Ellenőrzés: új `npm run test:gateway-request-context` (7 eset: NaN→undefined a régi regresszióra, tört/negatív/vegyes, UUID kis/nagybetű, csonka/nem-UUID, üres/null). A módosított fájlokon `tsc --noEmit` és célzott ESLint tiszta. A `/code-review` skill Standards+Spec tengelye lefutott: a Spec-ág mind a négy szándékot igazolta (NaN sosem csúszik át, a tenant-kötés valóban betölti és összeveti a `tenantId`-t, fail-safe `undefined`, nincs üzenet-szivárgás), blokkoló rést nem talált; mindkét ág egybehangzó észrevételét (a hibastátusz szövegre-illesztett osztályozása törékeny) a fenti `instanceof GatewayBudgetError` átállással feloldottam. (Branch: `fix/gateway-cost-record-integrity`, **PR nyitva**.)
