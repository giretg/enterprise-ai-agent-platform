...
- Fix applied:
  - Új közös primitív: `app/src/lib/agent-api-key-hash.ts` — `isAgentApiKeyFormat` (előtag-őr) és `deriveAgentApiKeyLookupHash` (szerveroldali kulccsal képzett determinisztikus HMAC-SHA-256 kereső-lenyomat). A nyugalmi titok TOVÁBBRA is bcrypt; a HMAC kizárólag gyors, egyedi-indexelt megkeresésre szolgál, és az adatbázis-szivárgás nem ad offline ellenőrzőt a nyers kulcshoz. A CodeQL jelzés dokumentáltan elnyomott, mert ez nem jelszó-tárolási hash.
