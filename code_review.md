...
## 2026-09-04 - Claude Code OAuth-bridge: a helyi credential-fájl túl tág jogosultsága kiszivárogtathatta az előfizetéses hozzáférést

- Áttekintett, korábban külön nem naplózott komponens: a **Claude Code OAuth-bridge** — `app/src/domain/gateway/claude-code-oauth-bridge.ts`, beleértve a `CLAUDE_CODE_OAUTH_EMBEDDED=true` fájlos fallbacket, a macOS Keychain-prioritást, a tokenfrissítést és a Messages API hívásait. Ez a platform Claude Pro/Max előfizetéséhez tartozó access- és refresh-tokent kezeli, tehát az agentek külső modellhez való hozzáférésének bizalmi határa.
- Rendben talált kontrollok: a környezeti token nem kerül fájlba; macOS-en a Keychain elsőbbséget kap; a frissítő- és modellhívás fix HTTPS-végpontra, időkorláttal fut; upstream hiba- vagy tokenszöveg nem kerül felhasználói hibaüzenetbe; a hívások előtt a Model Gateway adatvédelmi, érzékenységi, budget- és routing-kapui futnak.
- **Lelet (közepes, helyi titok-kiszivárgás):** a fájlos fallback beolvasta a `~/.claude/.credentials.json`-t annak jogosultságaitól függetlenül. A `writeFileSync(..., { mode: 0o600 })` sem szigorítja egy már létező fájl módját. Egy hibásan group/world-readable (például `0644`) credential-fájl így hosszú életű access- és refresh-tokent tehetett olvashatóvá a futtatógép más helyi felhasználói vagy más, azonos megosztott fiók alatti folyamatok számára.
- Javítás: közös, fail-closed jogosultság-kapu minden fájlos betöltés előtt (`stat.mode & 0o077` → elutasítás). Tokenfrissítésnél a fájl **már az új token kiírása előtt** `0600` módra áll; jogosultság-állítási hiba esetén nincs írás. A Keychain- és környezeti tokenes út változatlan.
- Üzleti hatás: egy megszerzett OAuth refresh tokennel illetéktelen személy a platform Claude-előfizetését használhatja, költséget okozhat és a modellhívási hozzáférést a szervezet nevében használhatja. A rendszer most inkább egyértelmű konfigurációs hibával leállítja ezt a provider-utat, mint hogy bizonytalan jogosultságú helyi titokra építsen.
- Ellenőrzés: új, Keychaintől független regressziós teszt bizonyítja, hogy a `0644` fájl elutasított, a `0600` fájl elfogadott. `npm run test:claude-code-oauth-bridge` (6/6), `npm run test:subscription-oauth-runtime-safety` (7/7), célzott ESLint, teljes `npx tsc --noEmit --pretty false` és `git diff --check` zöld. A `/code-review` Standards- és Spec-tengelye a végleges diffet is átnézi; lelet nélkül zárva kerül a PR-be.
