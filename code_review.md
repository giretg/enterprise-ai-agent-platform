...
- Ellenőrzés: új `app/scripts/sandbox-versioning.test.ts` zöld a content-addressed párhuzamos fa és a dupla jóváhagyás regressziójára; célzott ESLint zöld; `git diff --check` tiszta. A teljes `tsc --noEmit` csak a korábban már meglévő, nem ebbe a PR-be tartozó training-WIP hibákat jelzi (`training-workspace.test.ts`, `training-workspace.tsx`). A package.json eddig egy nem létező `test:sandbox-versioning` célra hivatkozott; a regressziós teszt ezt a CI-horgot is ténylegesen lefedi.
