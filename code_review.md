...
- Ellenőrzés: `npx tsx scripts/agent-interaction-ticket.test.ts` — 7/7 zöld; `npx tsc --noEmit` — teljes fa zöld; `eslint` az érintett fájlokra zöld. A `/code-review` mindkét ága lefutott: Standards „approve" (nincs kemény szabálysértés), Spec „no blocking defects" — a három invariáns teljesül, a gate a `resolveWorkOwnerTenantId(null, agent.tenantId)` révén helyesen az agent tenantjára kulcsol.
