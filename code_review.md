...
## 2026-09-03 - Recept-katalógus (RecipeService): a bérlő-elkülönítés és az audit-bélyeg hiányzott egy, az agent-promptba táplált governance-felületről

- Áttekintett, korábban külön nem naplózott komponens: a **recept-katalógus** — `app/src/domain/recipe/recipe-service.ts` (a szolgáltatás-regiszterben `services.recipes`), `app/src/repositories/postgres/recipe-repository.ts`, a `Recipe`/`RecipeVersion` modell és a recept futásidejű felhasználása (`app/src/domain/agent/wiki-runtime.ts`, `agent-repository.ts`: `agentVersion.recipeVersion.recipe`). A recept a tickettípus-szintű „hogyan végezd” utasítás hordozója, amelynek jóváhagyott tartalma bekerül az agent rendszer-promptjába.
- Rendben talált kontrollok: a recept-verziók jóváhagyás-kötelesek (proposed→active), a verzió-váltás retire-öli a korábbi aktívat, és a modell csak jóváhagyott verziót köt az agent-verzióhoz (reprodukálhatóság). A tartalom jelenleg kizárólag platform-szintű seed-recept (`key-ceremony`, `wiki-answer`), a runtime FK-relációval oldja fel — a jelen állapotban NINCS élő kihasználható rés.
- **Lelet (közepes, latens több-bérlős elkülönítés + auditálhatóság):** a `RecipeService` a szolgáltatás-regiszterben publikált, teljes API-felület (list/create/proposeVersion/approveVersion), de EGYETLEN metódusa sem kapott vagy kényszerített ki `tenantId`-t, és a `recipes` táblának NEM volt bérlő-oszlopa. A `list()` globálisan minden receptet visszaadott, a `findById`/`approveVersion` bérlő-kapu nélkül dolgozott, és a `recipe.create/version/approve` audit-események `tenantId` nélkül keletkeztek (attribúció nélkül). Ez ma dead/seed-only kód, de a felület pontosan az a fajta „később rákötünk egy szerkesztő UI-t” governance-komponens, amelynél a bérlő-rés bekerülése a legvalószínűbb — és mivel a kimenet az agent-promptba folyik, egy globális recept-halmaz egyenesen cross-tenant utasítás-szivárgás lenne.
- Javítás: a `recipes` bérlő-oszlopot kap (`tenant_id`, NULLABLE — a `0043_recipe_tenant_scope` migráció; NULL = platform-szintű globális sablon, ezért a meglévő seed-sorok biztonságosan maradnak, és minden bérlő agentje köthető hozzájuk). Új közös, pure szabályok (`recipe-tenant-scope.ts`) — az OLVASÁS és az ÍRÁS szándékosan eltér: `isRecipeReachableByTenant` (olvasás: globális NULL VAGY saját), és a SZIGORÚBB `isRecipeWritableByTenant` (írás: KIZÁRÓLAG saját — globális sablonhoz bérlő NEM adhat/nem hagyhat jóvá verziót, különben egy globális recept jóváhagyott tartalma minden bérlő agent-promptját megmérgezné). A repository listázása/olvasása a reachability-halmazra szűr, a `findById`/`getActiveVersion` fail-closed olvasáson, az `addVersion`/`approveVersion` fail-closed a szigorúbb ÍRÁS-kapun (idegen vagy globális recept „nem létezőként” viselkedik, nincs létezés-orákulum). A `RecipeService` minden belépőpontja kötelező `tenantId`-t kap, továbbadja a repositorynak ÉS az audit-eseménynek. (A `/code-review` Spec-tengely lelte a globális-sablonra írás rését; a Standards-tengely nyomán a négy repo-ellenőrzés a közös helpereken megy át.)
- Üzleti hatás: a több-bérlős elkülönítés és a teljes körű auditálhatóság a platform két alapígérete. A javítás biztosítja, hogy amikor a recept-katalógusra végül felhasználói felület épül, egy bérlő ne láthassa/módosíthassa más bérlő „hogyan végezd” utasításait (amelyek az agent viselkedését vezérlik), és minden recept-művelet a megfelelő bérlőhöz rendelve jelenjen meg az audit-nyomon. A globális, platform által gondozott sablonok továbbra is mindenki számára elérhetők maradnak.
- Ellenőrzés: `npx prisma generate`, teljes `npx tsc --noEmit` (0 hiba), új `npm run test:recipe-tenant-scope` (5/5 zöld — a pure reachability-szabály + a szolgáltatás tenant-továbbadása a repositoryra és az auditra). A migráció NULLABLE oszlopot ad hozzá backfill nélkül, ezért a meglévő (kizárólag seed-) sorokon nem bukhat; éles alkalmazása a szokásos `db:migrate:deploy` úton történik (élő DB-t ebben a futásban nem tudtam migrálni).
- PR: https://github.com/giretg/enterprise-ai-agent-platform/pull/422 (`fix/recipe-tenant-scope-clean` ág).

## 2026-09-02 - Agent API / MCP Tool Broker bridge: idegen ticket- vagy beszélgetésazonosítóval más tenant munkaterülete elérhetővé válhatott

- Áttekintett, korábban külön nem naplózott komponens: az **agent API-kulcsos Tool Broker belépési pont és a hozzá tartozó stdio MCP bridge** — `app/src/app/api/v1/agent/tools/route.ts`, `app/src/harness/platform-mcp-bridge.ts`, a kanonikus tool-regiszter, valamint a ticket-/beszélgetés-alapú workspace-tenant feloldás. Ez a külső harness/MCP-kliens útja a platform tényleges eszközeihez, ezért itt dől el, hogy egy ellopott vagy hibás agent-kulcs meddig juthat.
- Rendben talált kontrollok: az API-kulcs agenthez és scope-okhoz kötött; a broker capability- és connector-kapuja él; a delegált felhasználói identitást az external agent API nem fogadhatja el közvetlenül; a `board_write` végrehajtása korábban is ellenőrizte a ticket agent-tulajdonosát.
- **Lelet (magas, cross-tenant jogosulatlan workspace-hozzáférés):** a generikus agent tools API a kliens által küldött top-level `ticketId`/`conversationId` értéket ellenőrzés nélkül továbbadta. A file- és workspace-eszközök ebből választják ki a tárolási tenantot, miközben a broker csak néhány speciális eszköznél (pl. `board_write`) vizsgálta, hogy a ticket a hívó agenté-e. Egy érvényes agent API-kulccsal és egy ismert idegen azonosítóval így elméletben idegen ügyfél munkaterületének olvasása vagy írása indulhatott volna.
- Javítás: a tool API a tenant-státusz és a broker előtt fail-closed ellenőrzi, hogy **minden, végrehajtási kontextust vagy workspace-t választó** ticket és beszélgetés a kulcs agentjéhez tartozik-e. A `board_write` rejtett, argumentumban lévő cél-ticketje is ebbe a halmazba kerül. A Run Analyst saját argumentum-beli ticket-/beszélgetés-horgonyai nem ebbe a körbe tartoznak: azok csak tenanton belüli lekérdezési szűrők, és a diagnosztikai role-nak szándékosan más agent futásait is el kell érnie. A hiányzó vagy idegen végrehajtási kontextus ugyanazzal a nem részletező, magyar 403-mal bukik; a tiltott kísérletet a közös `recordDenied` út ToolCall-sora, append-only audit-eseménye és deny-metrikája is rögzíti, de idegen célobjektumot nem kapcsol az auditbejegyzéshez.
- Üzleti hatás: a tenant-szigetelés az enterprise AI platform egyik alapígérete. A javítás megakadályozza, hogy egy agent-kulcs a munkafolyamat-kontektsus manipulálásával egy másik ügyfél dokumentumaihoz vagy feladataihoz jusson. Emellett a biztonsági csapat most bizonyíthatóan látja a tiltott próbálkozásokat, anélkül hogy azok részletei visszaszivárognának a hívónak.
- Ellenőrzés: `node --import tsx scripts/agent-api-tool-context.test.ts` (5/5), `tool-broker-bridge`, `tool-broker-tenant-isolation`, célzott ESLint, teljes `npx tsc --noEmit --pretty false` és `git diff --check` zöld. A célzott tesztek saját, idegen és hiányzó ticketet, idegen/hiányzó beszélgetést, valamint a `board_write` top-level vs. argumentum cél-ticket eltérést fedik.

## 2026-09-02 - Tool Broker következmény-kapu (gmail_send emberi jóváhagyás): a jóváhagyó jegy nem volt a hívó agenthez/tenanthoz kötve + tetszőleges címzett-visszaesés

- Áttekintett, korábban külön nem naplózott komponens: a **Tool Broker következmény-kapu / emberi-jóváhagyás betartatása** (`app/src/domain/tool-broker/tool-broker-service.ts` `invoke` → `checkGmailSendApproval`), a `consequence-gate-policy.ts` és a `tool-broker-authorizer.ts`. Ez az a pont, ahol eldől, hogy egy AI agent önállóan (autonóm ticket/process úton is) végrehajthat-e egy magas következményű, kilépő műveletet — konkrétan e-mailt küldhet-e a felhasználó Gmail-fiókjából. A platform legfőbb ígérete, hogy az autonómia mértéke pontosan kontrollálható; ez a kapu ennek a magja.
- Rendben talált kontrollok: az `AllowlistAuthorizer` szerep-sablon → capability → connector-feloldás → tenant-izoláció → grant/scope láncot fut, `acting_user`-t követel user-delegált connectornál, felfüggesztett usert kizár; a `resolveActingUserId`/`resolveActingTenantId` sosem fogad el a kérés törzséből választott identitást (csak a saját ticket/conversation kontextusból); a `board_write` a jegyet a hívó agenthez köti; a `gmail_send` a chat-loopban következmény-kapuzott (kártya + audit-nyom). A jóváhagyást beállító `approveGmailSend` server action `approver` szerepet és tenant-scope-ot követel.
- **Lelet (magas, broken access control / confused deputy):** a `gmail_send` egyetlen érdemi kapuja az autonóm (agent-API, `/api/v1/agent/tools`) úton a `checkGmailSendApproval` volt, ez viszont az **agent által beküldött `approvalTicketId`-t** kötés nélkül fogadta el. A `tickets.findById` GLOBÁLIS, tenant-szűrő nélküli lookup, és a kapu csak azt nézte, hogy a jegy `approved`, és a `gmailSendApproved` mező egyezik-e a `draftId`-val (`?? args.to` visszaeséssel). Nem ellenőrizte, hogy a jegy a hívó agenté vagy a hívás tenantjáé — így egy kompromittált/prompt-injektált agent egy MÁSIK agent vagy MÁSIK tenant jóváhagyott jegyére hivatkozva oldhatta volna fel a saját küldését. A `?? args.to` visszaesés emellett egy draftId-jóváhagyás alapján tetszőleges `to` címzettet engedhetett (a jóváhagyó a KONKRÉT vázlatot hagyja jóvá, nem egy címzettet).
- Javítás (`tool-broker-service.ts`):
  - A jóváhagyó jegy **agent-kötése**: `ticket.agentId === input.agentId` (ugyanaz az invariáns, mint a `board_write`-nál/`resolveActingUserId`-nél).
  - **Fail-closed tenant-kötés**: az összevetés alapja az agent HITELES tenantja (agent-rekord, nem a kliens által befolyásolható `actingTenantId`); megosztott (platform, tenantId=null) agentnél a feloldott hívás-tenant a jel; ha egyik sem oldható fel, a küldés NEM futhat. (A `/code-review` Spec-tengelye mutatta ki, hogy a kezdeti `actingTenantId !== null` őr fail-OPEN volt — épp a nem hitelesített external-agent úton hagyta ki a tenant-ellenőrzést.)
  - A `?? args.to` visszaesés törölve: kizárólag a jóváhagyott `draftId` pontos egyezése old fel.
- A `/code-review` skill (Standards + Spec, párhuzamos sub-agentek) lefutott a diffen:
  - (Spec) BEÉPÍTVE: a fail-open tenant-őr → agent-tenant alapú, fail-closed kötés + célzott regressziós teszt a feloldatlan-tenant útra.
  - (Standards) nincs hard violation; megjegyzés: az agent-kötés-ellenőrzés több (~7) helyen ismétlődik — közös `ticketBelongsToCaller` helper összevonná; szándékosan a repo bevett (inline) konvencióját követtem, a security-fix ne bővüljön refaktorra.
  - NYITVA HAGYVA (külön szelet, viselkedés-változás kockázata): (1) **single-use / replay** — a jóváhagyás nincs elfogyasztva, ugyanaz a draftId elvben ismételten küldhető (a draft Gmail-oldali elfogyasztása ad de-facto védelmet, de a platform-invariáns „egy jóváhagyás = egy küldés" nincs kikényszerítve); (2) **tartalom-kötés** — a jóváhagyás a draft AZONOSÍTÓJÁHOZ köt, nem a tartalmához (utólag módosított draft-törzs/címzett mellett a régi jóváhagyás átmegy); (3) tágabb megfigyelés: a `gmail_send`-en KÍVÜLI `ALWAYS_CONSEQUENCE_GATED_TOOLS` (Drive share/trash, `file_delete`, `repo_open_pull_request`, sandbox promote/commit, `memory_propose`) következmény-kapuja csak a chat-loopban fut, az autonóm `invoke` úton nincs analóg kapu — külön policy-döntést igényel (l. korábbi 2026-07-31 megfigyelés).
- Üzleti hatás: e-mail-küldés a felhasználó nevében visszafordíthatatlan, kifelé irányuló művelet. A hiba mellett egy elszabadult vagy megtévesztett agent a saját autentikált kulcsával, egy máshol adott jóváhagyásra hivatkozva küldhetett volna levelet emberi rábólintás nélkül — pont az „AI csak jóváhagyással lép ki a rendszerből" ígéret ellen. A javítás a jóváhagyást a konkrét agenthez, tenanthoz és vázlathoz köti, és feloldatlan tenant mellett inkább tilt, mint enged.
- Ellenőrzés: `tool-broker-tenant-isolation` (+5 új gmail-kötés teszt: helyes kötés átmegy; más agent / más tenant / draftId-nélküli-`to` / feloldatlan-tenant tilt), `consequence-gate`, `consequence-approval` — mind zöld (`node --import tsx`/`tsx`, DB-mentes deny-utak); `npx tsc --noEmit` 0 hiba; célzott ESLint tiszta.

## 2026-08-30 - Google Drive user-delegált connector: a nem-admin felhasználó megszerezhette a TELJES Drive írási jogot (a Picker-korlát és az „admin only" kapu megkerülésével)

- Áttekintett, korábban külön nem naplózott komponens: az **új Google Drive user-delegált connector** teljes hozzáférés-/scope-rétege — `app/src/domain/connector-grant/google-drive-scopes.ts`, `google-drive-write-access.ts` (Picker-manifeszt kapu), `google-drive-grant-metadata.ts`/`-grant-store.ts`, a `delegated-oauth-registry.ts` scope-feloldás, a Tool Broker `google-drive.handler.ts` + authorizer, valamint a `connector-grants.ts` server actionök (Picker-session, kijelölés-mentés, OAuth-indítás) és a token-vault. Ez az a pont, ahol egy AI agent a felhasználó VALÓDI Google Drive-ján olvashat és írhat; itt dől el, hogy az „AI csak a kijelölt fájlokhoz fér hozzá" ígéret betartatható-e.
- Rendben talált kontrollok: a grant a `(tenant, connector, user)` hármashoz kötött, minden Picker-action `grant.userId === ctx.user.id`-t ellenőriz (nincs cross-user IDOR), a kijelöléseket a Drive-nál újra-validálja; a token-vault Secret Managerben / `0o600` dev-fájlban tárol, nyers token nem kerül DB-be, a token_ref path fertőtlenített; az authorizer `acting_user`-t követel, felfüggesztett usert kizár, és pre-flight scope-kaput futtat; a `share_file` és `trash_file` mindig következmény-kapuzott. Cross-tenant írási/olvasási rést nem találtam.
- **Lelet (magas, broken access control / jogosultság-emelés):** a legérzékenyebb Drive-profil, a **`full_write` (teljes `drive` scope)** a UI-ban `adminOnly`-ként van jelölve, de ezt a kaput **kizárólag a böngésző** (`connector-connection-card.tsx:91`) tartotta be. A `startConnectorOAuth` server action csak `viewer` szerepet kért, elfogadott kliens-oldali `scopes` tömböt, és azt csupán a connector-config **részhalmazának** ellenőrizte (`resolveRequestedScopes`) — szerepkör-vizsgálat nélkül. Mivel a seed Drive-config mindhárom scope-ot listázza (`readonly`/`file`/`full`), egy alacsony jogú felhasználó (vagy egy kompromittált session) a server actiont közvetlenül hívva megkaphatta a teljes `drive` scope-ot → `full_write` grant → a **Picker-manifeszt ellenőrzés teljesen kimarad** (`requiresSelectedWriteManifestCheck` csak `selected_write`-re fut), így az agent a felhasználó **teljes Drive-ján** írhatott/áthelyezhetett/megoszthatott/kukázhatott. Az „admin only" felirat puszta UI-dísz volt.
- **Lelet (magas, ugyanabban az útban — csendes eszkaláció mindenkinek):** a tool-vezérelt „Hozzáférés megadása" folyamat (`connector-grant-needed-panel`) `resolveGrantOAuthScopes({toolName})`-t hív, amely a `driveToolMinimalScopes` visszaadta `[file, full]` metszetét kérte — így **minden** Drive-toolnál (még a **csak-olvasó** `google_drive_read_file`-nál is) a **teljes `drive`** scope-ot kérte el. Empirikusan igazolva: mindhárom próbált tool (`read_file`, `update_file`, `share_file`) `full_write` profilra oldódott fel. Következmény: a Picker-alapú „kijelölt fájlok" modell a **fő úton mindenkinél** hatástalan volt — egyetlen nem-admin „Hozzáférés megadása" kattintás után az agent az egész Drive-ot írhatta.
- Javítás (a termékígéretet — „olvasás/írás csak a kijelölt fájlokon; teljes írás csak admin" — kényszeríthetővé teszi):
  - `driveToolMinimalScopes`: olvasás → `drive.readonly`, írás → `drive.file` (selected_write). A teljes `drive` scope-ot egyetlen tool sem kéri automatikusan — az kizárólag explicit admin-döntés.
  - Új `driveScopeProfileRequiresAdmin`, amely UGYANABBÓL a `DRIVE_SCOPE_PROFILES.adminOnly` metaadatból dolgozik, amit a UI is használ (egy forrás-igazság, nem tud szétcsúszni).
  - `startConnectorOAuth` szerveroldali kapu: a teljes-írás Drive-scope kérése **tenant-admint követel**; a nem-admin közérthető üzenetet kap, amely a selected_write profilra és a Picker-kijelölésre irányítja.
- Build-higiénia (a review során talált, a feature-alapvonalon MÁR meglévő hibák — nem az én regresszióm): a `tool-registry.ts`-ben a `google_docs`/`google_slides` apply_edits descriptor nem fordult (`Record<string,unknown>[]` vs. `Docs/SlidesEditOperation[]`) → nominális cast, a művelet-alakot a `GoogleWorkspaceApiClient` futásidőben validálja; a `tool-registry-drift.test.ts` allowlistjébe felvéve a két connector-grant Drive tool-lista (olvasás/írás scope-kategorizálás + manifeszt-alkalmazhatóság — önálló szemantika, mint a `consequence-gate-policy.ts`); a `connector-grant-needed.test.ts` drift-őre `['gmail']`-t várt, holott a feature regisztrálta a `google_drive` providert.
- A `/code-review` skill (Standards + Spec tengely, párhuzamos sub-agentek) lefutott a teljes feature-diffen; a leleteiből BEÉPÍTVE:
  - (Spec) a `selected_write` a specben `drive.readonly` + `drive.file` — az első javításom írástoolra csak `drive.file`-t kért, ami egy friss granton elrontotta volna a keresést/olvasást; javítva a `[readonly, file]` párra (teljes `drive` továbbra sincs benne).
  - (Standards, fail-closed) a manifeszt-kapu `if (fileId && …)` mintája hiányzó fájl-id mellett ÁTENGEDTE az írást (a `share_file`-t is); most fail-closed (üres id → elutasítás).
  - (Standards, drift) az allowlist-mentesítés helyett/mellett új VISELKEDÉSI drift-teszt: minden mellékhatásos Drive-tool ténylegesen manifeszt-kapuzott kell legyen (üres/nem-kijelölt argsra dob) — így egy jövőbeli, kezelésből kihagyott írástool bukik, nem csúszik némán fail-openbe.
  - (Standards) a docs/slides cast kommentje pontosítva (a futásidejű ellenőrzés csak a felső-szintű kulcsot nézi, a beágyazott mezőket a Google bírálja).
- A `/code-review` NYITVA HAGYOTT (külön feature-munka, nem e PR): (Spec) a full-write ne csak „a kérő admin", hanem **connectoronkénti admin-opt-in** legyen (GD-7.6/9.1.9); (Spec) a callback után a **ténylegesen visszaadott scope-ok ellenőrzése + részleges granular-consent → read-only visszaminősítés** (§5.3/GD-3.4); (Standards) a három kézzel karbantartott write-tool lista (handler/write-access/scopes) összevonása; a `getGoogleDrivePickerSession` `localhost:3000` origin-fallbackja prod-guardot érdemel.
- Megfigyelés (nem javítva, terméklogika): a mutáló, de nem-kilépő Drive-tartalomtoolok (`update_file`, `docs_apply_edits`, `sheets_write_range`, `slides_apply_edits`, `move`/`rename`/`copy`) **nem** következmény-kapuzottak — csak a `share_file` (kiszivárgás) és a `trash_file` (törlés) az. A javítás után a nem-admin alap-útja `selected_write`, így ezek a felhasználó által kifejezetten kijelölt fájlokra korlátozódnak; ez védhető terméklogika, de a „felülírás = a korábbi tartalom megsemmisítése" kockázatot érdemes külön mérlegelni.
- Üzleti hatás: egy enterprise AI platform legfőbb ígérete, hogy az AI pontosan a kapott hozzáféréshez fér — nem többhöz. E hibák mellett a legszűkebb jogú felhasználó (vagy egy ellopott session) az agentnek a **teljes vállalati Google Drive** írási jogát adhatta, megkerülve a kijelölt-fájlos korlátot és az admin-jóváhagyást. A javítás a hozzáférés-kaput a böngészőből a szerverre teszi, és a fő „Hozzáférés megadása" utat least-privilege scope-ra állítja, így a kijelölt-fájlos modell ténylegesen érvényre jut.
- Ellenőrzés: `google-drive-scopes` (új regressziók: a tool-minimum SOHA nem tartalmaz teljes `drive`-ot; a seed-config mellett sem eszkalál `full_write`-ra; `driveScopeProfileRequiresAdmin` csak a `full_write`-ra igaz), `google-drive-write-access`, `connector-grant-needed`, `delegated-oauth-ui`, `tool-registry-drift`, `consequence-gate`/`-approval`, `tool-broker-tenant-isolation` — mind zöld (`node --import tsx`, IPC-mentes; a `npm run test:*` a sandbox `tsx` EPERM-je miatt nem indul). `npx tsc --noEmit` az egész app-ra tiszta (a fenti build-higiénia után 0 hiba); célzott ESLint és `git diff --check` tiszta. (Branch: `fix/drive-full-write-admin-server-gate`, **PR #401 NYITVA**.)

## 2026-08-27 - Tanítási munkaterület és tartós szabályverziók: párhuzamos jóváhagyás elhasználhatta a write-gate tokeneket és meghamisíthatta volna az audit-nyomot

- Áttekintett, korábban külön nem naplózott komponens: a **Tanítási munkaterület / betanított működési szabályok** teljes írási útja — `app/src/domain/training/*`, a `PostgresTrainingStore`, a `WriteGateService`-kapcsolat, a Control Plane server actionök és a Tanítás UI. Ez az a pont, ahol egy operátor vagy jóváhagyó egy agent tartós munkaszabályát módosítja; ezért itt dől el, hogy az AI működése visszakövethetően és kontrolláltan változik-e, vagy észrevétlenül driftelhet.
- Rendben talált kontrollok: a server actionök minden belépéskor hitelesítenek és szerepkört kérnek; az agent és ticket tenant-határa fail-closed; a preview HMAC-aláírt, rövid életű, felhasználóhoz és tenanthez kötött; az aktiválás újraellenőrzi a szerepkört, a four-eyes policyt, az aktuális revíziót, a bázisverziót, a profil scope/diff-limitjét és a kemény jogosultsági padlót. A tényleges memória-mutató compare-and-set művelete már korábban is csak egy írót engedett; a rollback ugyanígy CAS-védett. A szabály- és projektmemória-láncok szétválasztása is megmarad.
- **Lelet (magas, integritás / auditálhatóság):** két külön böngészőből vagy szerverpéldányból indított, ugyanazon revízióra érkező jóváhagyás a memória-mutató CAS-a miatt végül csak egyszer írt új szabályverziót, de **a CAS előtt mindkét kérés kiadhatott és elfogyaszthatott saját egyszer használatos write-gate tokent**. A vesztes kérés ráadásul felülírhatta a revízió és a training ticket tokenhivatkozását. Emiatt az audit a nem ténylegesen élesítő tokenre mutathatott volna, miközben egy másik token volt a valódi memóriaírás engedélye. Ez nem adatvesztés, de egy banki/enterprise ügyfélnek éppen a bizonyítható „egy jóváhagyás → egy konkrét, visszakereshető szabályverzió” láncot teszi kétségessé; dupla kattintás vagy két approver ugyanazt a döntést látszólag több jóváhagyási eseménnyé tehette.
- Javítás: a revízió `unissued → issued` állapotváltása most atomi **aktiválási foglalás**. Csak a foglalást megnyerő kérés futtat evalt, kér write-gate tokent és írhat memóriát. A többi kérés közérthető `activation_in_progress` választ kap. Egy foglalt jóváhagyást új szerkesztés vagy visszautasítás sem cserélhet le félúton; sikertelen, még memóriaírás nélküli próbálkozásnál a foglalás felszabadul, így a javaslat nem ragad be és újrapróbálható. Siker után a tokenhivatkozás csak azután kerül a revízióra/a ticketre, hogy a memória CAS valóban megtörtént.
- Üzleti hatás: a tanítás a platform fő governance-ígérete: az ügyfélnek vissza kell tudnia mondani, **ki, melyik szabályváltozást, milyen engedéllyel tett élessé**. A módosítás megakadályozza, hogy egy hálózati újrapróbálás, dupla kattintás vagy két approver egyidejű döntése több, egymásnak ellentmondó engedélynyomot hozzon létre. Így az audit és a ténylegesen futó agent-szabály újra egyértelműen összetartozik.
- Ellenőrzés: `node --import tsx scripts/training-workspace.test.ts` zöld (19 eset, ebből 2 új regresszió: párhuzamos jóváhagyás → 1 token/1 memóriaírás; foglalt jóváhagyást sem szerkesztés, sem elutasítás nem írhat felül); `training-tenant-boundary`, `durable-memory-policy`, `training-composition` és `training-teach-plan` céltesztek zöldek; célzott ESLint, `npx tsc --noEmit --pretty false` és `git diff --check` zöld. A szokásos `npm run test:training-workspace` a sandboxban a `tsx` IPC socket-nyitásának `EPERM` hibája miatt nem indul, ezért ugyanazt a tesztet `node --import tsx` IPC-mentes módban futtattam.

## 2026-08-24 - Futás-elemző (`run_analysis`): a platform által ígért egress-tilalom capability-drifttel megkerülhető volt

- Áttekintett, korábban külön nem naplózott komponens: a **Futás-elemző (Run Analyst)** teljes, tenant-szintű futási napló-elemző útja — `app/src/domain/run-analysis/*` (`run_index`, `run_trace`, `run_stats`), a hozzá tartozó system-role és materializáció (`app/src/domain/agents/run-analyst-role.ts`, `app/src/domain/agent-access/run-analyst-materialization.ts`), a Tool Broker authorizer/handlerek, valamint az „Elemezd” belépési kapu. Ez a komponens a tenant AgentTurn/ToolCall/ModelCall, ticket- és folyamat-futási nyomából ad magyarázatot az adminnak, ezért üzletileg hasznos hibadiagnosztikai felület, ugyanakkor a benne lévő naplók érzékeny ügyfél- és működési adatokat hordozhatnak.
- Rendben talált kontrollok: a `run_index` minden explicit és felfedező scope-ankert tenant szerint ellenőriz és idegen azonosítóra `run_not_found` választ ad; a turn/ticket/process `run_trace` ág ugyanígy tenant-scope-olt, a system agent saját futásait kizárja; a `run_stats` a feloldott scope nélküli teljes-tenant lekérdezést üres eredménnyel zárja; az elemző csak admin granttal címezhető, alapértelmezett user-grantot nem kap. A nyers naplók külső modell felé menő útján a sensitivity-policy is aktív.
- **Lelet (magas, adatbizalmasság / policy-drift):** a role-sablon és a specifikáció szerint a Futás-elemző csak `run_index`, `run_trace`, `run_stats` és `ticket_create` capability-t kaphat; egress tooljai (web, Gmail, HTTP API, repo) tiltottak. Ez azonban csak a **provisioning pillanatában** volt igaz. A normál `updateAgentCapabilities` admin szerverakció system-role megkülönböztetés nélkül írt capability-sort, a Tool Broker pedig ezt általános capability-ként elfogadta. Így egy tenant admin — akár téves konfigurációval, akár kompromittált admin-sessionnel — bekapcsolhatta a `gmail_send`, `http_api_request` vagy `web_search` eszközt a napló-elemzőn. Az agent ezután a futási trace-ekből megismert ügyféladatot külső rendszerbe küldhette volna, megsértve a feature kimondott „nincs egress” bizalmi határát. Az `outboundRestricted` agent-gráf kapcsoló ezt nem védi: az agentek közötti elérhetőséget szabályozza, nem a Tool Broker tool-engedélyt.
- Javítás, három egymást erősítő szinten:
  - A role egyetlen futásidejű allowlistet ad (`isRunAnalystToolAllowed`); a Tool Broker ezt a capability-sor **előtt** ellenőrzi, ezért egy közvetlen adatbázis-módosítás vagy jövőbeli adminfelület sem indíthat tiltott toolt (`system_role_tool_not_allowed`).
  - A capability-szerkesztő a `run_analyst` system role módosítását auditáltan elutasítja. Az admin érthető jelzést kap, hogy ez platform által védett szerep, nem csendben változó konfiguráció.
  - A materializáció következő futásakor minden nem kanonikus, korábban esetleg aktív capability-t letilt, majd helyreállítja a négy kötelező jogot.
- Üzleti hatás: a Futás-elemző értéke, hogy az ügyfél a saját AI-futásait biztonságosan megérthesse. Egress-képességgel viszont a hibadiagnosztikai eszköz maga válna a legkényelmesebb adatkiszedési csatornává. A módosítás a termékígéretet („elemzi, nem továbbítja”) kényszeríthetővé teszi akkor is, ha egy adminbeállítás hibás vagy egy capability-adat driftel.
- Ellenőrzés: `node --import tsx scripts/run-analyst-role.test.ts` zöld (a regresszió explicit engedélyezett `web_search` és `gmail_send` capability mellett is tiltást vár); célzott ESLint zöld; `npx tsc --noEmit --pretty false` zöld. A `npm run test:run-analyst-role` parancs a sandboxban a `tsx` IPC socket-nyitásának `EPERM` hibája miatt nem indítható, ezért ugyanazt a tesztet a `node --import tsx` IPC-mentes futtatásával ellenőriztem. (Branch: `fix/run-analyst-capability-egress-lock`; PR: a GitHub DNS-feloldási hibája miatt helyben előkészítve, még nem nyitható.)

## 2026-08-22 - Platform-beállítások szolgáltatás (`platform-settings-service.ts`): a nyers PAN/IBAN kiengedés elavult „superadmin-kapu" maradványa + auditálhatatlan nyoma

- Áttekintett, korábban külön nem naplózott komponens: a **Platform-beállítások szolgáltatás** (`app/src/domain/platform-settings/platform-settings-service.ts`, ~2000 sor, egyetlen fájl) — a futásidőben állítható, platform-/tenant-/agent-szintű vezérlők központi rétege: dispatcher/monitor kill-switch, model-policy + fallback-lánc + árazás, egress-allowlist bővítés, DB-mód váltás és prod→teszt szinkron, Google OAuth platform-titok, valamint az **AI Privacy Gateway** kategória-policy és sensitivity-réteg beállítói. A tenant-scope-os bucketek helyesen tenantId-kulcsoltak, a mutációk auditáltak és validáltak; az egress-allowlist bővítés (`extendEgressAllowlist`) `matchForbiddenHost`-tal védett (nyers IP/localhost/metadata tiltva), a Google OAuth `clientSecret` nem szivárog az auditba. Cross-tenant írási/olvasási rést nem találtam.
- **Lelet (közepes, megtévesztő biztonsági garancia + holt kód):** a nyers **PAN/IBAN külső modellnek küldése** (`pan`/`iban` → `allow`) körüli kapu **fél lábon maradt** a #320 **D9** döntés után. A D9 **tudatosan** megszüntette ennek a superadmin-jogát, és a platform-szint eltűnésével tenant-admin hatáskörbe tette, a visszatartó kontrollt pedig a **gépelt megerősítésre + audit-eseményre** bízta („Vállalt döntés; audit-esemény és a UI-n külön megerősítés marad"). A kód azonban ezt csak félig vezette át:
  - Az `allow_requires_superadmin` hibakód (`privacy-category-policy.ts`) definiálva volt, a szerver-action (`privacy.ts`) **kezelte is** egy **tényszerűen hamis** üzenettel („Bankkártyaszám vagy IBAN külső modellnek küldését csak platform-admin engedélyezheti."), de a domain **sosem dobta** — holt ág + hazug biztonsági dokumentáció épp a legérzékenyebb PII-egress úton. A docstring (`privacy-category-policy.ts:18`) továbbra is „csak superadmin"-t állított.
  - A category-policy **audit-esemény** (`recordPrivacyCategoryPolicySet`) csak az érintett kategória **nevét** rögzítette, a megválasztott **akciót nem** — a naplóból nem derült ki, hogy egy változás a nyers pénzügyi adat **kiengedését** (`allow`) kapcsolta-e be vagy épp szigorított. Így az a kontroll, amelyre D9 a superadmin-kapu helyett kifejezetten **támaszkodott**, a gyakorlatban nem töltötte be a szerepét.
- Javítás (a D9 DÖNTÉST NEM vonja vissza, hanem igazságossá teszi és megerősíti a megtartott kontrollt):
  - A holt `allow_requires_superadmin` kód és a hozzá tartozó hamis-üzenetű action-ág **eltávolítva**; a docstring a D9 valóságához igazítva (tenant-admin + gépelt megerősítés + audit). Az assert actor-típusa `confirmation`-re szűkítve (az `isSuperadmin` itt már nem kapu — nem sugallja tévesen a superadmin-jogot).
  - Az **audit-esemény kibővítve**: rögzíti a megválasztott akciókat (`actions`), és külön kiemeli a nyers-egress bekapcsolást (`rawEgressEnabled` lista + `policyDecision: 'category_policy_raw_egress'`), így a SIEM/megfelelőségi lekérdezés meg tudja különböztetni a szigorítást a nyers PAN/IBAN kiengedésétől, és riasztani tud rá.
- Üzleti hatás: egy enterprise AI platform megfelelőségi (PCI/GDPR) története azon áll, hogy a nyers bankkártyaszám/IBAN külső modellhez engedése **kivételes, megerősített és utólag kimutatható** legyen. A D9 döntés jogosan tette tenant-admin hatáskörbe, de a hozzá ígért audit-nyom nem mutatta meg a tényleges kockázatot, a kód pedig hamisan superadmin-kaput dokumentált. A javítás megszünteti a megtévesztést és a megtartott audit-kontrollt ténylegesen működőképessé teszi.
- Megfigyelés (nem javítva, tudatos/tesztelt döntés — külön termékdöntést igényel): a **legacy** `agents.allow_sensitive_external_model` kapcsoló feloldáskor a `secret_key`-t is `allow`-ra állítja (`resolvePrivacyCategoryPolicy`, tesztelt: `privacy-category-policy.test.ts:180`), holott a mentési invariáns a `secret_key`-t sérthetetlenül `block`-ban tartja — így a deprecated migrációs úton nyers API-titok/privát kulcs mehet külső modellhez. A viselkedés a régi „mindent-vagy-semmit" kapcsoló megőrzése; érdemes külön kivezetni.
- Ellenőrzés: `test:privacy-category-policy` 15/15 zöld (2 új regressziós teszt az audit-tartalomra: `actions` + `rawEgressEnabled` + `policyDecision`, és a `null`→`inherit` naplózás). `tsc --noEmit` tiszta. A `/code-review` skill Standards+Spec tengelye lefutott. (Branch: `fix/privacy-pan-iban-egress-audit-and-stale-superadmin-gate`, **PR #342 NYITVA**.)

## 2026-08-21 - AI Privacy Gateway (APG-21 debug-trace projekció): a nyers PAN/IBAN/titok/TAJ nyersen ment ki a hibakereső AI-hoz

- Áttekintett, korábban külön nem naplózott komponens: az **AI Privacy Gateway** (`app/src/domain/privacy/*`, ~6300 sor) — a PII-osztályozó/álnevesítő réteg, amely a külső LLM-provider felé menő tartalmat védi. Végigvizsgáltam a kimenő álnevesítést (`prompt-privacy-transform.ts`, `known-value-matcher.ts` Aho–Corasick + magyar toldalék-tűrés, `apply-replacements.ts` span-csere), a visszafejtő utat (`resolve-tool-args.ts`, `surrogate-engine.resolveRef/resolveVal`), a hozzáférés-kaput (`resolve-access.ts` — tenant + scope + résztvevő, erős, cross-tenant rést nem találtam), a HMAC-hitelesítést (`surrogate-vault.ts`, timing-safe), a kripto-borítékot (`aes-gcm-envelope.ts` AES-256-GCM random IV) és a kategória-policyt (`privacy-category-policy.ts`). A tenant-izoláció és a fő prompt-út fail-closed viselkedése rendben van.
- **Lelet (magas, adatszivárgás / dokumentált garancia megsértése):** a **debug-trace projekció** (`debug-trace-projection.ts`, APG-21) — amely az agent-forduló nyomát pszeudonimizálva adja át egy külső **hibakereső AI-nak** — csak azokat a kategóriákat álnevesítette, amelyek akciója `tokenize` vagy `local_only` ÉS van álnév-típusuk (`company`/`person`/`email`/`phone`/`account`). Minden más **nyersen** átment. Emiatt a legérzékenyebb adatok — **bankkártyaszám (PAN), IBAN, API-titok (`sk-…`/privát kulcs), TAJ, adószám** — nyersen kerültek a hibakereső modell contextjébe. Ez pontosan fordított a helyes rangsorral: a `block`/`forbidden` kategória a legszigorúbb, mégis védtelen maradt, míg a gyengébb (`tokenize`) védve volt. A modul dokumentált Definition-of-Done-ja („a debug-trace tool kimenetében nincs nyers entitásérték") sérült; a meglévő teszt csak cégnév/e-mail szivárgást fedett, PAN/IBAN/titkot nem. Empirikusan igazolva (repró: mind a 4 kategória nyersen átment; javítás után mind redaktált/álnevesített).
- **Lelet (közepes, ugyanabban az útban):** egy álnév-típusú kategória `block`-ra állítása (pl. admin a `phone`-t a legszigorúbbra állítja) **kevésbé** védett, mint a `tokenize`: a `block` kimaradt a `tokenize`/`local_only` szűrőből, így a telefonszám nyersen ment ki — a szigorítás gyengített.
- Javítás:
  - **Minden nem-`allow` kategória védelmet kap a debug-trace egressen.** A szűrő logikája megfordult: már nem a védendő kategóriákat sorolja fel (`tokenize`/`local_only`), hanem csak az egyetlen kivételt engedi nyersen (`action === 'allow'` — az admin kifejezett hozzájárulása). Aminek van álnév-típusa → pszeudonimizálás (a `block` is, feloldható a trusted zónában); aminek nincs (PAN/IBAN/titok/TAJ/adószám) → **visszafordíthatatlan redakció** `«redaktált:…»` jelölővel.
  - **Közös redakciós jelölő.** Új, exportált `redactionMarkerForCategory` (`sensitivity-router.ts`), amelyet a kimenő content-guard (`redactSensitiveText`) és a debug-trace is használ — a naplók és a trace egységes jelölőt mutatnak, nincs formátum-drift.
- Üzleti hatás: egy enterprise AI platform legfőbb ígérete, hogy ügyfél-PII és titkok nem szivárognak külső modellekhez. A hibakereső-AI funkció (APG-21) pont azért pszeudonimizál, hogy a nyomot külső modell láthassa — de a nem-álnevesíthető, legérzékenyebb adatokra (kártyaszám, IBAN, API-kulcs, társadalombiztosítási szám) lyukas volt: ezek nyersen mentek ki. A javítás bezárja a rést (redakció ott, ahol álnév nincs), és helyreállítja a rangsort (a `block` legalább annyira véd, mint a `tokenize`).
- Ellenőrzés: 2 új regressziós teszt a `scripts/debug-trace-projection.test.ts`-ben (PAN/IBAN/titok/TAJ redakció; `phone`→`block` álnevesítés). A teljes `test:debug-trace-projection` zöld (10 eset), valamint a kapcsolódó `chat-thinking-trace`, `privacy-prompt-order`, `model-gateway-negative`, `privacy-end-to-end`, `memory-content-guard` változatlanul zöld. `tsc --noEmit` és célzott ESLint tiszta a módosított fájlokon. A `/code-review` skill Standards+Spec tengelye lefutott (nincs blokkoló hiba; a review finomításait — ternária-olvashatóság, komment a nyers vs. kanonikus kategóriáról, `block`-úti konzisztencia-teszt — beépítettem). (Branch: `fix/privacy-debug-trace-nontokenizable-redaction`, **PR #319 NYITVA**.)

## 2026-08-15 - Workspace fájl-letöltés: a rejtett (belső) agent-fájlok is letölthetők voltak nyers `?path=…`-sal

- Áttekintett, korábban külön nem naplózott komponens: a **workspace fájl-tár letöltési felülete** — a `WorkspaceStorage` (`app/src/domain/file-editor/workspace-storage.ts`), a fájl-láthatóság policy (`app/src/lib/workspace-file-visibility.ts`) és a két HTTP letöltő route: `app/src/app/api/v1/tickets/[id]/workspace/files/route.ts` és `app/src/app/api/v1/conversations/[id]/workspace/files/route.ts`. Ezek szolgálják ki a chat/ticket felületén a fájllistát, a letöltést, az inline HTML-előnézetet és a 15 perces aláírt GCS letöltő-URL-t. A tenant-határ (a ticket/conversation `tenantId`-je + `viewer` szerep) mindkét route-on erős — cross-tenant rést nem találtam.
- **Lelet (magas, broken access control / adatminimalizálás — tenanton belül):** a fájl**lista** (`listUserFacing` → `isWorkspaceFileUserFacing`) SZÁNDÉKOSAN elrejti a felhasználó elől az agent belső munka-fájljait: az `audience='internal'`-ra jelölteket és a technikai útvonalakat (`.workspace-meta/`, `.tool-results/`, `tool-outputs/`, `*_raw.json` / `*_extract.json` / `*_response.json`). A **letöltő** ág (stream + aláírt-URL + inline) viszont a nyers `?path=…` lekérdezés-paramétert vette, és ezt a döntést NEM ismételte meg: bármely path lefolyt, csak a tenant + `viewer` kapun át. Következmény: egy tenant legalacsonyabb szerepkörű felhasználója (aki a path-t ismeri vagy kitalálja — a listában nem látszó, de determinisztikus nevű nyers tool-kimenetek, kivonat-JSON-ok) letölthette a szándékosan rejtett belső fájlokat, köztük a nyers, esetleg PII-t tartalmazó köztes adatokat és a `.workspace-meta` audience-markereket. A „rejtett" jelző így puszta UI-dísz volt, nem tényleges hozzáférés-korlát. (Az aláírt-URL ág súlyosbító: az egyszer kiadott URL a route megkerülésével, közvetlenül a GCS-ről tölthető.)
- **Lelet (közepes, defense-in-depth):** a letöltő route-ok KÖZVETLENÜL a `WorkspaceStorage`-ot hívták, megkerülve a `FileEditorService.resolveSafePath` `..`-szűrését — a tár maga nem normalizált. GCS-en a kulcs literál (a `..` nem lép ki a prefixből), de a lokális/stub úton és bármely jövőbeli tár-adapternél ez valódi útvonal-kilépés lehetne. A path-biztonság csak az agent-oldali szolgáltatásban élt, a HTTP-oldalon nem.
- Javítás:
  - **Letöltés-kapu = lista-kapu.** Új közös helper `resolveDownloadableWorkspacePath` (`app/src/domain/file-editor/workspace-download-guard.ts`): normalizál, feloldja a fájl közönség-besorolását (`WorkspaceStorage.getFileAudience` — a listázással AZONOS marker→legacy forrásból), és a listázással azonos `isWorkspaceFileUserFacing` döntést hozza. A nem-felhasználói fájl `404` (nem `403` — a végpont ne legyen létezés-orákulum a rejtett fájlokra). MINDKÉT route MINDHÁROM ága (stream, aláírt-URL, inline) ezen a kapun megy át. A közös helper feloldja a `/code-review` Standards-tengely „a döntés két belépési ponton duplikálva" leletét.
  - **Tár-szintű útvonal-kapu.** Új, exportált `safeObjectPath` a `WorkspaceStorage`-ban, bekötve minden kulcsépítő metódusba (`read`/`write`/`delete`/`streamToClient`/`getSignedDownloadUrl`/`getFileSize`/`get`+`setFileAudience`). A tár mostantól minden hívónál (route ÉS szolgáltatás) elutasítja a `..`-kilépést. A `FileEditorService` korábbi, karakterről-karakterre azonos `resolveSafePath` másolata megszűnt — egyetlen biztonsági primitívre delegál (a Standards-tengely „egy biztonsági primitív két példányban" leletének feloldása).
  - **Marker-konzisztencia:** a `setFileAudience` és a `getFileAudience` a NORMALIZÁLT path-ra kulcsolja a base64url markert, így egy `./x`-re állított jelölés is párosul az `x` letöltéssel.
- Spec-illeszkedés (a `/code-review` Spec-tengely megerősítette): mindhárom cél teljes, nincs szivárgás, megkerülés vagy tenant-regresszió; jogszerű felhasználói letöltés (jelöletlen `report.html`/`report.xlsx`, ill. `audience='user'`) változatlanul működik — a listázás és a letöltés bizonyíthatóan ugyanazt a függvényt hívja. Az egyetlen viselkedésváltozás a tenant-perem alatt: a nyers path-t közvetlenül a tárnak átadó három hívó (`domain/index.ts` sandbox content-resolver, `agent-chat-runtime`/`general-task-runtime` `readWorkspaceFile`) egy `..`-nél most `FileEditorError`-t kap — de mindhárom fail-soft (a két runtime `try/catch`→`null`, a resolver eddig is dobott „not found"-ot). Nincs funkcionális regresszió.
- Üzleti hatás: egy enterprise AI platformon a fájl-besorolás (ki mit láthat/tölthet le) valódi hozzáférés-korlát, nem UI-díszítés. E hiba mellett a legszűkebb jogú belső felhasználó is kimásolhatta az agent nyers, rejtett munka-fájljait — köztük olyan köztes adatokat, amelyeket a governance szándékosan tartott távol a felhasználói felülettől. A javítás összehangolja a letöltést a listázással (rejtett = tényleg elérhetetlen), és a tárba magába teszi az útvonal-kaput, hogy jövőbeli hívó se tudja megkerülni.
- Ellenőrzés: új `npm run test:workspace-download-guard` (8 invariáns: `..`-elutasítás a tár peremén read/write/delete/stream-en; `.`/dupla-slash normalizálás; `audience='user'` → letölthető; `audience='internal'` → 404; marker nélküli technikai path → 404; rejtett path normalizált aliasa is 404; nem-normalizált úton beállított marker is párosul). A meglévő `test:workspace-file-visibility`, `test:workspace-io-performance`, `test:workspace-resource-access`, `test:file-search-safe`, `test:xlsx-range-delete`, `test:tulajdoni-lap` változatlanul zöld. `tsc --noEmit`: az érintett fájlok tiszták (a jelentkező `skill-service.ts` hibák MEGLÉVŐK, Prisma-kliens drift — l. alább, e PR-en kívül). Célzott ESLint tiszta. A `/code-review` Standards+Spec tengely lefutott; mindkét ág leletét (két duplikáció, ill. spec-teljesség) e körben feloldottam. (Branch: `fix/workspace-file-download-audience-guard`, **PR #262 NYITVA**.)
- Megjegyzés (e PR-en kívül): a `tsc --noEmit` 3 MEGLÉVŐ hibát jelez a `skill-service.ts`-ben (`Property 'attachments' does not exist` a StepTemplate/SkillVersion típuson) — Prisma-generate/migrate drift (0028 migráció), a változtatásaimtól független. Külön karbantartás.

## 2026-08-14 - Web-kutatási delegáció (web_research_request): a delegált webes egress audit és napi keret NÉLKÜL futott

- Áttekintett, korábban külön nem naplózott komponens: az **agent→agent web-kutatási delegáció** egress-útja (`web_research_request`). Teljes függőleges metszet: a tool-handler (`app/src/domain/tool-broker/handlers/web-research.handler.ts`), a keret-metódus (`webResearchRequest` a `app/src/domain/tool-broker/tool-broker-delegation.ts`-ben), a web-egress role feloldás (`resolveWebEgressAgent` → `selectActiveTenantWebEgress`), a determinisztikus eredmény-validátor (`web-research-validator.ts`), a közös SSRF/allowlist-őr (`app/src/domain/net/egress-guard.ts`), a tényleges letöltő (`WebFetchService.fetch`, `content-sanitize.ts`) és a bekötés (`app/src/domain/index.ts`).
- Biztonsági értékelés összességében: **erős** — a webes kimenet fail-closed opt-in (perzisztált `systemRole=web_egress` principal + agent→Web-Egress `address` grant a gráfban), a fetch csak `web_search`-találatokban szereplő URL-re megy (URL-invariáns), az egress-őr deny-by-default allowlisttel + feloldás-utáni privát-IP re-checkkel (DNS-rebinding) véd, a tartalom sanitizált és a fogyasztónál adat (nem utasítás), a validátor pedig tipizált, hash-only kontraktust kényszerít. Ezen a tengelyen érdemi rést nem találtam.
- **Lelet (közepes, audit-lefedettség / defense-in-depth keret-megkerülés):** a delegációs úton a tényleges egress-letöltéseket kiszolgáló `webResearchFetch` bekötés (`domain/index.ts`) a `WebFetchService.fetch`-et **audit és napi keret-könyvelés nélkül** hívta. A `WebFetchService` szándékosan „pure" (nem auditál, nem számol keretet) — ezt a hívónak kell megtennie, és a testvér, felfedező (provisioning) út meg is teszi (`web_fetch.request`/`web_fetch.blocked` hash-only audit + a napi számláló táplálása). A delegációs closure viszont csak **beolvasta** a napi számlálót (`web_fetch.request` count 24h-ra), de **sosem írt** ilyen eseményt. Következmények: (1) a delegált webes egress-letöltések **láthatatlanok** a `web_fetch.*` audit-nyomvonalban (compliance-rés); (2) mivel a napi keret (`perAgentDay`, alap 50) számlálója pont ezeket az eseményeket olvassa, a keret **soha nem érvényesült** a delegációs úton — a védelmi rétegből kicsúszott; (3) a blokkolt (SSRF/allowlist/budget) kísérletek **nem** generáltak `web_fetch.blocked` monitoring-jelet, így egy belső IP-t próbálgató agent némán maradt.
- Spec-illeszkedés: a hiba **spec-drift** volt, nem szándékos kihagyás. A WebEgress-Delegáció spec §9.2 kimondja: „A meglévő `web_fetch.request`/`web_fetch.blocked` **változatlan** — a fetch-szintű audit alattuk fut, mint ma", és §9.3: az általános (delegációs) úton „az elsődleges korlát a `maxFetchesPerAgentDay`". A javítás pontosan ezt a spec által előírt, de az implementációból hiányzó viselkedést állítja helyre.
- Javítás:
  - Új közös, **kötelezően auditált** egress-nyelő: `app/src/domain/web-fetch/audited-web-fetch.ts` (`performAuditedWebFetch`). Egy helyen köti össze a napi keret beolvasását, a letöltést és a hash-only auditot: minden kísérlet — sikeres ÉS blokkolt — `web_fetch.request`/`web_fetch.blocked` eseményt kap (`actor = web-egress role agent`, spec §9.2), és minden sikeres letöltés beleszámít a napi keretbe. A számláló hibája fail-soft (0-ra esik).
  - **MINDKÉT** egress-út erre az egyetlen nyelőre került: a web-kutatási delegáció ÉS a felfedező (provisioning) `runWebFetch` is (utóbbi a role/capability-kapu UTÁN). A korábban duplikált count+fetch+audit törzs megszűnt — így a két út audit-alakja már nem tud újra szétcsúszni (a `/code-review` Standards-tengely „Duplicated Code" leletének feloldása).
  - A delegációs út napi plafonja a fixen bedrótozott 50 helyett a **konfigurálható** `webFetchBudgetMax.perAgentDay`-t használja (spec §9.3: „az elsődleges korlát a maxFetchesPerAgentDay") — így egy admin által csökkentett keret ezen az úton is hat (a `/code-review` Spec-tengely lelete). A per-request forrás-korlát a hívó `maxSources` slice-a marad (spec §9.3).
- Üzleti hatás: egy enterprise AI platformon minden kifelé irányuló hálózati hívás auditálandó és keretezendő. E hiba mellett az agentek a delegált „web-kutatás" csatornán át a napi egress-keret felett, a naplóban nyomtalanul tölthettek le külső tartalmat, a belső hálózat próbálgatása pedig nem hagyott riasztható jelet. A javítás visszaadja a teljes egress-láthatóságot (hash-only, adatvédelem-barát) és érvényre juttatja a napi keretet ezen az úton is — ugyanazzal az egyetlen, közös nyelővel, ami megakadályozza a jövőbeli újra-szétcsúszást.
- Ellenőrzés: új `npm run test:audited-web-fetch` (4 eset: sikeres→`web_fetch.request` hash-only, nyers URL nem szivárog; blokkolt→`web_fetch.blocked`; a napi felhasznált keret átmegy a fetch-be; számláló-hiba fail-soft). A meglévő `test:web-research`, `test:web-fetch`, `test:egress-guard`, `test:web-discovery`, `test:provisioning`, `test:provisioning-governance`, `test:egress-allowlist` változatlanul zöld (a provisioning-út a közös nyelővel is). `tsc --noEmit`: az érintett fájlok tiszták (a jelentkező `skill-service.ts` hibák MEGLÉVŐK, Prisma-kliens drift — l. lentebb, e PR-en kívül). Célzott ESLint tiszta. A `/code-review` skill Standards+Spec tengelye lefutott, és mindkét ág leletét (provisioning-egyesítés + konfigurálható keret) e körben javítottam. (Branch: `fix/web-research-egress-audit-budget`, **PR #258**.)
- Megjegyzés (e PR-en kívül): a `tsc --noEmit` 3 MEGLÉVŐ hibát jelez a `skill-service.ts`-ben (`Property 'attachments' does not exist` a StepTemplate/SkillVersion típuson) — ez Prisma-generate/migrate drift, a változtatásaimtól független (stash-elve is fennáll). Külön karbantartás.

## 2026-08-12 - Rendszer-agent Registry: névütközéses tenant-adat- és módosítási határ

- Áttekintett, korábban külön nem naplózott kritikus komponens: a **platform-szintű rendszer-agent Registry** és annak modellkonfigurációs admin útja — `app/src/lib/platform-agent-registry.ts`, `app/src/app/actions/system-agents.ts`, a Platform-beállítások „Rendszer agentek” felülete, valamint a közös `applyAgentModelConfigUpdate` auditált író útja. Ezek a Playbook Author és Provisioning Assistant közös, minden tenant által használt háttéragentjeinek modelljét kezelik.
- Kiinduló védelem: a hozzáférési gráf helyesen csak akkor tekint egy agentet panel-varázslónak, ha a kanonikus neve **és** `tenantId = null` is teljesül; ezt az Access Policy specifikáció is előírja (egy közös platform-példány).
- **Lelet (magas, cross-tenant bizalmasság és integritás):** az új rendszer-agent admin action ettől eltérően csak a szerkeszthető `name` mezőt ellenőrizte. Egy tenant admin létrehozhatott saját „Playbook Author” vagy „Provisioning Assistant” nevű agentet. Ez a tenant-agent bekerülhetett a platform-auditorok globális listájába (modellkonfigurációval és azonosítóval együtt), majd egy superadmin a rendszer-agent formon keresztül át is írhatta a modellbeállítását. A platform-szerep így egy névütközés miatt kilépett a valódi platform-agentek köréből; a szerveroldali írás a kliens által küldött ID-ra ment.
- Javítás:
  - A rendszer-agent lista már adatbázis-szinten `tenantId: null` szűrést kap, majd ugyanazt a közös `isPanelWizardAgent` predicate-et használja, mint az access-policy.
  - A módosítás cél-agentjét a repository is kötelező `tenantId = null` scope-pal tölti be, és csak utána fogadja el panel-varázslóként. Azonos nevű tenant-agent ezért nem listázható és nem módosítható ezen a globális úton.
  - `selectSystemPanelWizardAgents` közös helper és regressziós teszt rögzíti, hogy a platform-példány bekerül, a tenantbeli névazonosító viszont kimarad.
- Üzleti hatás: a rendszer-agentek modellválasztása befolyásolhatja, hogyan készülnek a Playbook- és connector-javaslatok minden ügyfélnél. Nem fogadható el, hogy egy ügyfél saját, azonos nevű agentje a globális üzemeltetői felületen jelenjen meg vagy ott legyen módosítható. A javítás visszaállítja az egyértelmű tulajdonosi határt: a platform admin csak a közös platform-példányokat látja és kezeli, az ügyfél agentje pedig minden esetben saját tenantjában marad.
- Ellenőrzés: `npm run test:system-agent-registry` — zöld (platform-varázsló engedett, tenantbeli névazonosító kizárt); `npx tsc --noEmit` és célzott ESLint a módosított TypeScript fájlokra. A review a Access Policy §„Platform-agentek” invariánsát ellenőrizte: a dedikált panel-varázsló kizárólag közös `tenantId = null` példány lehet. (Branch: `agent/harden-system-agent-registry`, **PR #250**.)

## 2026-08-11 - StepTemplate admin CRUD (Governed Flow Builder): eval-minta írás cross-tenant IDOR-ja a verzió-ID-n

- Áttekintett, korábban külön nem naplózott komponens: a **StepTemplate admin CRUD** (`app/src/domain/step-template/step-template-service.ts` + a `app/src/app/actions/step-template.ts` server-actionök). Ezek a Governed Flow Builder újrafelhasználható lépés-sablonjait (a Playbook-canvas építőkövei) kezelik: tenant-scope-os és globális (seed, `tenantId=null`) sablonok, verziózással és opcionális „certified" eval-tierrel. A teljes mutációs metszetet átnéztem: `create/update/publish/retire/delete/setEvalSamples/certify`.
- Tenant-izolációs értékelés összességében: **jó** — minden sablon-szintű mutáció a közös `requireEditable` tulajdon-kapun megy át (globális sablon read-only a tenant-adminnak, idegen tenant sablonja „nem található"). A `create` kulcs-ütközést is tenant-scope-ban néz.
- **Lelet (magas, cross-tenant írás / multi-tenant izoláció-törés):** a `setStepTemplateEvalSamples` a `template` tulajdonjogát ellenőrizte (`requireEditable(id, tenantId)`), majd a **kliens által beküldött `versionId`-vel** vakon írt: `stepTemplateVersion.update({ where: { id: input.versionId } })` — a verzió sablonhoz-tartozását sosem ellenőrizte. Így egy tenant-admin a saját (jogszerű) `id`-jét megadva egy MÁSIK tenant vagy a **globális seed-katalógus** verziójának ID-jét célozhatta meg, és annak `evalSamples` mezőjét tetszőleges JSON-ra írhatta (vagy üresre törölhette). Súlyosbító: a globális sablonok verzió-ID-jei minden tenant-adminnak láthatók a `listStepTemplatesForAdmin` admin-listán (`versions[].id`) — nem kellett ID-t találgatni, közvetlenül kihasználható volt. A testvér `certifyStepTemplateVersion` ugyanezt a mintát MÁR helyesen kötötte (a verziót a `template.versions`-ből oldja fel), ez tette egyértelművé, hogy a `setEvalSamples` kihagyta a kötést.
- Javítás:
  - Közös `requireOwnedVersion(prisma, id, tenantId, versionId)` helper: a verziót IS a tulajdon-ellenőrzött `template.versions` közül oldja fel, ismeretlen `versionId` → `StepTemplateError('A verzió nem található.')`. Mind a `setStepTemplateEvalSamples`, mind a `certifyStepTemplateVersion` erre a helperre került → egyetlen verzió-író út se felejtheti el az újra-feloldást (a `/code-review` Standards-tengely „Duplicated Code" leletének feloldása is).
  - Elavult docstring javítva („a legfrissebb verzióhoz" → a hívó saját sablonjához kötött, megadott verzióhoz).
- Üzleti hatás: a lépés-sablon-katalógus governance-eszköz — a „certified" jelzés és az eval-minták adják a bizalmi szintet, amire a tenantok a Playbook-építéskor hagyatkoznak. E hiba mellett egy ügyfél admin(ja) a MÁSIK ügyfél, vagy a minden tenant által látott globális seed-katalógus eval-mintáit csendben átírhatta/kitörölhette — hamis vagy lerontott minőségi jelzést hagyva a katalógusban. Ez a többbérlős izoláció alapszabályát töri egy írási úton; a javítás visszaállítja a „minden verzió-mutáció a saját sablon saját verziójára szól" invariánst.
- Ellenőrzés: új `npm run test:step-template-isolation` (4 eset: cross-tenant verzió-ID elutasítva + a globális minta érintetlen; saját verzió → mentés lefut; idegen template-id a tulajdon-kapun bukik; certify-horgony a saját verzióhoz kötött) — a pre-fix kód ellen a cross-tenant eset MEGBUKNA. `tsc --noEmit` és `eslint` az érintett fájlokra tiszta. A `/code-review` skill Standards+Spec tengelye lefutott: a Spec-ág megerősítette, hogy nincs másik kötetlen verzió-mutáció és a happy-path megmarad; a Standards „Duplicated Code" + elavult-komment leleteket a közös helper + docstring-javítás zárta. (Branch: `fix/step-template-eval-samples-cross-tenant-idor`.)

## 2026-08-11 - Csatornás emberi jóváhagyás: versenyhelyzet a kettős döntésnél / folyamat kétszeres léptetése

- Áttekintett, korábban külön nem naplózott komponens: az **eseményvezérelt csatornás emberi jóváhagyás** teljes döntési útja — `app/src/domain/channel/channel-approval-service.ts` (Telegram-gomb aláírása, címzetthez kötése, élő IAM-ellenőrzés, egyszer-használat), `channel-approval-token.ts`, `app/src/app/api/channels/telegram/webhook/route.ts`, a prompt-tár (`channel-repository.ts`), az éles bekötés (`domain/index.ts`), valamint a közös `TicketStateMachine` és a Playbook-folyamatléptetés. Ez a felület agent vagy folyamat következményes műveletének emberi jóváhagyását indíthatja el külső üzenetből.
- Ellenőrzött és rendben talált védelmek: a Telegram callback webhook-titka konstans idejű összehasonlítással védett; a gomb HMAC-kel aláírt, konkrét tickethez, kapuhoz és csatorna-identitáshoz kötött; a callbackkor a címzett aktív kötése és aktuális tenant-szerepe újraellenőrzött; a kezdeményező nem hagyhatja jóvá a saját kérését; a prompt beváltása egyedi és atomikus; más felhasználó, hamis vagy visszajátszott payload nem lépteti az állapotgépet.
- **Lelet (magas, integritás / működési megbízhatóság):** két jogosult kolléga közel egy időben megnyomott gombja (vagy egy Telegram-gomb és a webes döntés) ugyanazt a még `awaiting_human` ticketet olvashatta. A közös `TicketStateMachine` a policy-ellenőrzés után feltétel nélküli `update`-et végzett. Így mindkét kérés engedélyezettnek látszhatott, mindkettő sikeres auditot írhatott, és mindkettő meghívhatta a folyamat `advance` ágát. Üzletileg ez két egymásnak ellentmondó döntést, megkettőzött következő lépést vagy pontatlan compliance-auditot jelenthet egy kifizetés, ügyfélkommunikáció vagy más kontrollált művelet jóváhagyásánál.
- Javítás: a ticket-tár új `updateIfCurrentState` compare-and-set művelete adatbázisban csak akkor ír, ha a ticket még a korábban kiolvasott állapotban van. A `TicketStateMachine` ezt használja: kizárólag a nyertes döntés rögzít transition/audit sikeres eseményt és indíthatja el a folyamatléptetést. A vesztes kérés `STALE_TICKET_STATE` tiltási auditot és egyértelmű „időközben már eldöntötték” választ kap; nincs második hatás.
- Ellenőrzés: `npm run test:playbook-v2-process` zöld (új determinisztikus P5a regresszió: két ugyanazt a régi állapotot olvasó jóváhagyásból pontosan egy sikeres); `npm run test:channel-approval` zöld; `npx tsc --noEmit` és célzott ESLint zöld; `git diff --check` tiszta.

## 2026-08-06 - Agent-hozzáférési gráf: elérhetőségi kúp hamis „ciklus” jelzése (governance-nézet megbízhatatlanság)

- Áttekintett, korábban külön nem naplózott komponens: az **agent-hozzáférési gráf** (issue #142) teljes függőleges metszete — a tiszta policy-mag (app/src/lib/agent-access-graph.ts), a domain-service (app/src/domain/agent-access/agent-access-service.ts), a grant-tár tranzakciós tenant-invariánsa (app/src/repositories/postgres/agent-access-grant-repository.ts), a Web-Egress principal materializáció/kiválasztás (web-egress-materialization.ts, tenant-web-egress-selection.ts), a platform-agent registry (platform-agent-registry.ts), és MINDEN chokepoint: chat-indító (agent-chat-runtime), agent→agent delegálás/ticket-címzés/web-kutatás (tool-broker-delegation), prompt-roster (addressable-colleagues), felderítő toolok (agent_catalog/agent_resolve), a „suitable” REST-végpont és az admin szerkesztő server actionök (agent-access.ts).
- Tenant-izolációs értékelés: **erős**. A tenant-határ a policy-magban minden grant-adat ELŐTT dönt (fail-closed), a grant-írás cross-table tenant-invariánsa tranzakcióban ellenőrzött, minden chokepoint hiányzó dependency-nél zárt (üres lista / AGENT_NOT_FOUND). Ezen a tengelyen érdemi rést nem találtam.
- **Lelet (közepes, governance-megbízhatóság / téves riasztás):** az elérhetőségi kúp (`reachableAgentIds`) `hasCycle` jelzője hamisan igazat adott minden GYÉMÁNT/DAG-mintára — azaz valahányszor ugyanaz az agent két külön kollégán át is elérhető (reális szervezetben szinte mindig, pl. egy hub-agent). Ok: a BFS ugyanazt a csomópontot többször is sorba tette, és a második kivétel „már láttam → ciklus”-ként könyvelte, holott a többszöri FELFEDEZÉS nem irányított kör. Az admin org-ábra emiatt gyakorlatilag folyamatosan azt üzente, hogy „két agent kölcsönösen elérheti egymást”. Egy governance-nézet, ami folyton farkast kiált, elveszti a hitelét — és akkor egy VALÓDI delegációs kör (ami futásidőben token-elszállást/kör-futást okozhat, l. a visszaolvasás-livelock incidenst) észrevétlen marad.
- Javítás: a kúp számítása kettévált — (1) elérhető halmaz + legrövidebb-út mélység egyszeri-felfedezésű BFS-sel; (2) valódi ciklus-detektálás három-színes (fehér/szürke-nyílt-út/fekete-lezárt) iteratív DFS-sel, ahol csak egy MÉG NYITOTT úton lévő csomópontra visszamutató él számít körnek. A gyémánt-minta így helyesen NEM ciklus, a valódi kölcsönös elérés (a↔b) továbbra is az. A szerver és a kliens org-ábra ugyanazt a függvényt hívja, ezért egy helyen javult mindkettő.
- Üzleti hatás: az admin „ki kivel beszélhet / meddig ér el egy jog” kockázati nézete megbízható lett — a téves ciklus-figyelmeztetés eltűnik, a valódi kölcsönös-elérési kör pedig továbbra is látszik. Ez a hozzáférési blast-radius reális megítélését adja vissza a compliance-felelősnek.
- Ellenőrzés: `npm run test:agent-access-graph` — zöld (új regressziós esetek: gyémánt/DAG → nincs ciklus + helyes mélység; valódi a↔b kör → ciklus; a meglévő 6-hop lánc-ciklus teszt változatlanul zöld); célzott ESLint tiszta.
## 2026-08-06 - Debug-log export: nyers ügyfél- és dokumentumtartalom a támogatási csomagban

- Áttekintett, korábban külön nem naplózott komponens: az admin **Debug Log export** teljes útja — app/src/app/actions/debug-log.ts, app/src/domain/debug-log/debug-log-export-service.ts, app/src/domain/debug-log/debug-log-export.ts, a ConversationService/ticket-comment betöltés és a kapcsolódó debug-log-export regressziós teszt. Ez a support- és incidenskezelésnek szánt, böngészőbe letöltött JSON-adatkapu.
- **Lelet (magas, adatvédelmi/szivárgási kockázat):** a felület és a kód „telemetria” exportot ígért, a generikus toJsonSafe azonban a betöltött Prisma-objektumok MINDEN mezőjét szerializálta. Emiatt a csomagba bekerült a chat user- és agent-üzenetek nyers tartalma (tehát a modell completionje), ticket-kommentek és structured payloadok, agent partialText/hibák, ticket payload, valamint a kommentcsatolmányok Document.extractedText teljes fájlszövege és storageRef tárolási hivatkozása. Az admin-jogosultság nem teszi ezt biztonságos általános adat-exporttá: egy hibajegy továbbadása vagy helyi gépre mentése indokolatlan, nehezen követhető ügyféladat-másolatot hoz létre, miközben a futási hiba okához rendszerint elég az állapot, idő, korrelációs ID, mérés és policy-döntés.
- Javítás:
  - A debug-export séma **v2** lett, és minden rekord egyetlen, fail-closed toDebugLogSafe határon megy át.
  - A nyers chat-/modell-/ticket-/dokumentumtartalom, csatolmányfájlnév és storage-ref, runtime lock/access token, request/response/payload/metadata mező alapból kimarad. Új, jövőbeli Prisma-reláció sem kerülhet automatikusan a support-csomagba, ha ilyen mezőt hordoz.
  - A megmaradó rövid állapotjelző szövegek a közös érzékenyadat-redaktoron futnak, ezért e-mail, IBAN, kártyaszám vagy felismerhető titok sem jelenik meg olvashatóan.
  - A csomag továbbra is elég a működési hiba elhárításához: ID-k, időpontok, státuszok, számlálók, model/tool metrikák és governance döntések megmaradnak.
- Üzleti hatás: a support gyorsan diagnosztizálhatja, mi romlott el anélkül, hogy egy „Debug-log” gomb teljes ügyfélbeszélgetést vagy feltöltött szerződést másolna ki a kontrollált adatkezelési körből. Ez csökkenti a jogosulatlan belső megosztás, elveszett laptop és harmadik félnek küldött hibajegy kockázatát, és tisztább compliance-határt ad az enterprise ügyfeleknek.
- Ellenőrzés: npm run test:debug-log-export, npx tsc --noEmit, célzott ESLint és npm run build — zöld. A Next build a már létező next.config.ts NFT-trace figyelmeztetéseket jelzett, de sikeresen elkészült.
...
- **Lelet 4 (magas, rendelkezésre állás — a merge előtti utóellenőrzésen találva, PR #221):** a timeout-hiba besorolása a hibaüzenet SZÖVEGÉN múlt, az üzenet pedig beleírja a beállított ezredmásodperc-értéket. A `classifyProviderError` a kódmintákat puszta részszöveg-egyezéssel keresi, ezért egy 40 000, 4 000 vagy 240 000 ms-os időkorlát üzenete („timed out after 240000ms”) a `400` mintára esett → `content_error` → **nem fallback-képes**. Vagyis épp abban a helyzetben, amiért a timeout bekerült (beragadt szolgáltató), a tartalék-lánc NEM indult volna el, és a tenantok agentjei megálltak volna — a 120 000 ms-os alapérték csak véletlenül kerülte el az ütközést. Ugyanez érintette a Gateway → sidecar generikus `Model provider request timed out after …ms` hibát is.
  - Javítás: a besorolás a kódminták keresése előtt kiveszi az üzenetből a puszta időtartamot (`\b\d+\s*ms\b`), így egy konfigurációs szám soha nem viselkedhet hibakódként. A valódi HTTP-státuszok besorolása változatlan.
  - Regresszió: a runtime-safety teszt új esetei rögzítik, hogy a timeout 10/4 000/40 000/90 000/120 000/240 000 ms-nál is `provider_unavailable` és fallback-képes, valamint hogy a 401/429/400/404/502 státusz besorolása nem mozdult.
- Nyitott / követendő (nem-cél ebben a PR-ben): a `fetchWithProviderTimeout` időkorlátja a válasz FEJLÉCÉIG él, a törzs olvasására nem — egy fejlécet küldő, majd a JSON-törzset megakasztó sidecar így elvileg még mindig várakoztathat. A gyakorlatban a sidecar saját 120 s-os bridge-időkorlátja határolja, de a helyes megoldás a helper törzs-olvasásig kitartó időkorlátja; ez minden szolgáltatói utat érint, ezért külön változtatás. Továbbá a tokenfrissítés folyamaton belül sincs single-flight módon sorosítva, és a Secret Manager addVersion nem ad elosztott CAS-t. Ha az OAuth-szolgáltató refresh-token rotációt követel, két külön App Hosting példány egyszerre próbálhat azonos refresh tokennel frissíteni; ezt külön, verziózott secret + compare-and-retry vagy dedikált token-broker terve mellett kell kezelni.
...
## 2026-08-26 - Sandbox verziózás és test→live promóció: párhuzamos műveletek felülírhatták a jóváhagyott artefaktot vagy megduplázhatták az élesítést

- Áttekintett, korábban önállóan nem naplózott komponens: a **SandboxVersioning / Graduation** teljes kód- és adatsíne — `app/src/domain/sandbox-versioning/*`, a Tool Broker `sandbox.commit` / `sandbox.request_promotion` belépése, a Control Plane action/UI, a Postgres repository és a Prisma metaadatmodell. Ez a réteg választja el az AI által szabadon módosítható tesztkódot a csak ember által jóváhagyható éles változattól, ezért egy integritási hiba közvetlenül hibás vagy nem auditálható go-live-ot jelenthet.
- Rendben talált kontrollok: a tenant-határ fail-closed (`SANDBOX_NOT_FOUND_OR_FORBIDDEN` + access-denied audit); az agent nem hagyhat jóvá promóciót, nem exportálhat és nem érhet el live snapshotot; a promóció előtti live-adatmentés kötelező; a rollback új commitot hoz létre, nem írja át a történetet; a kód- és adattartalom auditba csak hash/referencia formájában kerül.
- **Lelet #1 (magas, kód-integritás):** két egyidejű commit a leendő `seq` becsléséből ugyanazt a GCS object-kulcsot (`…/commits/{seq}/tree.json`) kapta. Eltérő commit-tartalomnál az utolsó feltöltés felülírhatta az első fát, miközben az adatbázisban maradó commit még az első fa hash-ére/auditjára mutatott. Ez azt eredményezhette, hogy a jóváhagyó által ellenőrzött diff és a később tesztelt/promótált valós kód eltér — vagyis a go-live kontroll bizonyítéka megszakad.
- **Lelet #2 (magas, go-live integritás):** a promóció-jóváhagyás „olvasd a pending sort → snapshot → live pointer írás” lépésekből állt atomos foglalás nélkül. Két párhuzamos kattintás vagy szerverpéldány mindkét kérése készíthetett pre-promotion mentést és sikeres auditot; a test-fej ellenőrzése és a live pointer írása között beérkező új commit pedig régi test-fát élesíthetett.
- Javítás:
  - A commit-fa object-kulcsa most content-addressed (`tree_hash`), ezért két külön fa soha nem osztozik írható GCS kulcson. Azonos tartalom esetén az idempotens megosztás biztonságos.
  - A commit beszúrása és a `head`/`test` pointerek mozgatása egy project-szintű Postgres tranzakcióba és `FOR UPDATE` lock mögé került. Ez sorosítja a `seq` kiosztást és megakadályozza, hogy lassabb kérés régi fejre írjon vissza.
  - A promóció-döntés compare-and-settel (`pending_approval` → `approved`/`rejected`) foglalható le; a versenyvesztő nem készít snapshotot és nem auditál sikert. A végső live pointer frissítés újabb, atomos „még ez a test-fej?” összehasonlítással történik; stale esetben a promóció elutasított, auditált marad.
  - **Utókövetés a lefoglalás két mellékhatására.** (a) A lefoglalás a döntés indokát is felülírta, ezért az indok nélkül megnyomott „Jóváhagyom” kitörölte a *kérelmező* indokát a promóció-rekordból — pedig a go-live kérést jellemzően az agent indokolja, a jóváhagyó pedig indok nélkül kattint. Mostantól a kérelmező indokát csak a jóváhagyó saját, kitöltött indoka írhatja felül. (b) Ha a folyamat a lefoglalás után, a kötelező pre-promotion mentés közben megszakadt, a promóció véglegesen `approved` állapotban ragadt: sem befejezni, sem újra jóváhagyni nem lehetett, csak kézi adatbázis-beavatkozással. A megszakadt jóváhagyás 5 perc után újra lefoglalható (`reclaimStalledApproval`), a `decided_at` lease-ként előre tolódik, így két egyidejű újrapróbálásból is csak egy megy tovább; a türelmi időn belül a felület továbbra is „már folyamatban” választ ad.
- Üzleti hatás: a sandbox értéke, hogy az AI gyorsan építhet, de az üzleti felhasználó pontosan azt a változatot engedi élesbe, amit megvizsgált. A régi versenyhelyzet ezt a bizonyosságot bontotta meg: nagyobb terhelésnél a jóváhagyás nem feltétlenül a jóváhagyott fára vonatkozott, illetve ugyanaz a go-live kétszer szerepelhetett a compliance-bizonyítékban. A javítás a jóváhagyást a konkrét, megváltoztathatatlan tartalomhoz és egyetlen döntési folyamathoz köti.
- Ellenőrzés: új `app/scripts/sandbox-versioning.test.ts` 5/5 zöld — content-addressed párhuzamos fa, dupla jóváhagyás, indok-megőrzés, megszakadt jóváhagyás befejezése és a türelmi időn belüli újraindítás tiltása; célzott ESLint zöld; `git diff --check` tiszta. A teljes `tsc --noEmit` csak a korábban már meglévő, nem ebbe a PR-be tartozó training-WIP hibákat jelzi (`training-workspace.test.ts`, `training-workspace.tsx`). A package.json eddig egy nem létező `test:sandbox-versioning` célra hivatkozott; a regressziós teszt ezt a CI-horgot is ténylegesen lefedi.
