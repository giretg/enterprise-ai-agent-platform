...
## 2026-08-08 - Connector provisioning sandbox connection-test: hiányzó feloldás-utáni SSRF re-check (DNS-rebinding / belső-IP hostnév)

- Áttekintett, korábban külön nem naplózott komponens: a **connector provisioning sandbox connection-tester** teljes egress-útja — `app/src/domain/provisioning/sandbox-connection-tester.ts` (a tényleges kifelé menő próbahívás), a közös SSRF/egress-őr `app/src/domain/net/egress-guard.ts` (host-minta + privát/reserved IP osztályozás + `guardEgressUrl` DNS-rebind re-check), a bekötés `app/src/domain/index.ts`, és a hívó `extendEgressAllowlist` authz-kapu (`app/src/app/actions/provisioning.ts`, `requireTenantRole('admin')`). Cél: valóban élesben, multi-tenant enterprise platformon biztonságos-e a „connector kipróbálása” gomb.
- Kontextus-értékelés: a többi kifelé menő nyelő (`web_fetch`, `http_api` kliens, csatorna-kimenet, connector self-update spec-sync) mind a közös `guardEgressUrl`-en megy át, ami séma-korlát + allowlist + **feloldás-utáni privát/reserved IP re-check**-et (DNS-rebinding elleni védelem) végez. A platform-szintű authz-réteg (dispatcher/DB-mód/model-policy/web-kapcsolók) konzisztensen `requirePlatformRole('superadmin')`-nel, a tenant-scope-os kapcsolók `requireTenantRole('admin')` + `activeTenantId`-vel védettek — ezen a tengelyen rést nem találtam.
- **Lelet (közepes, SSRF / bizalmi határ átlépés):** a sandbox connection-tester az EGYETLEN egress-nyelő, amelyik NEM a közös `guardEgressUrl`-t hívta, hanem kézzel írt, gyengébb ellenőrzést: allowlist-tagság (jó) + statikus `isForbiddenHost` host-minta (nyers IP / localhost / metadata-literál / ismert exfil-sink). Hiányzott a feloldás-utáni IP re-check. Egy tenant admin (aki NEM platform-operátor, hanem az ügyfél oldali szereplő) felvehet az egress-allowlistre egy ártatlannak látszó **hostnevet**, amely belső címre (RFC1918, loopback, vagy a felhő-metadata `169.254.169.254`) oldódik fel — a statikus host-minta ezt nem fogja meg, mert a tiltott mintát csak a szó szerinti IP-literálra illeszti, nem a feloldott címre. A „Kapcsolat tesztelése” ezután a platform Cloud Run egress-éből GET-et intéz a belső címre. A válasz `detail`-je sanitizált (nincs törzs-szivárgás), így ez elsősorban **vak** SSRF: belső hálózat elérhetőség-térképezése és státuszkód-alapú port-/endpoint-szondázás a platform bizalmi zónájából.
- Javítás: a sandbox-tester mostantól — a `web_fetch`-csel közös `isPrivateOrReservedIp` őrrel — a próbahívás ELŐTT feloldja a cél-hostnevet és blokkol, ha bármelyik feloldott IP privát/reserved (`resolved_private_ip`), ha a feloldás hibázik (`dns_resolution_failed`) vagy nincs cím (`dns_no_address`). A feloldó (`resolveHostIps`) injektálható és a determinisztikus teszt hálózat nélkül fut; élesben az `index.ts` ugyanazt a `dns.lookup({all:true})` feloldót köti be, mint a többi nyelő. Feloldó nélkül a viselkedés bit-azonos marad (back-compat). Így a védelem egyetlen forrásból, minden kifelé menő úton egységes.
- Üzleti hatás: egy ügyfél tenant-adminja nem használhatja a connector-kipróbálást a szolgáltató belső hálózatának vagy felhő-metadata végpontjának szondázására — a platform és az ügyfél közti bizalmi határ ezen a felületen is zárt. Ez a fajta cross-tenant/cross-trust SSRF a valós, éles enterprise üzemeltetés egyik tipikus kockázata.
- Ellenőrzés: `npm run test:provisioning` — zöld (4 új eset: allowlistolt hostnév belső IP → `resolved_private_ip` hívás nélkül; RFC1918 → blokk; publikus IP → átengedve; feloldó nélkül back-compat), `npm run test:egress-guard` és `npm run test:egress-allowlist` — zöld, `npx tsc --noEmit` — 0 hiba.
