...
- Ellenőrzés: `tool-broker-tenant-isolation` (+5 új gmail-kötés teszt: helyes kötés átmegy; más agent / más tenant / draftId-nélküli-`to` / feloldatlan-tenant tilt), `consequence-gate`, `consequence-approval` — mind zöld (`node --import tsx`/`tsx`, DB-mentes deny-utak); `npx tsc --noEmit` 0 hiba; célzott ESLint tiszta. (Branch: `fix/gmail-send-approval-binding`, PR nyitás folyamatban.)
