...
## 2026-09-01 - Következmény-kapu jóváhagyó kártya: a jóváhagyó VAKON engedélyezhetett kimenő levelet / Drive-megosztást (a legveszélyesebb, adat-kivivő hívásoknál nem látszott, MI és KINEK megy ki)

- Áttekintett, korábban külön nem naplózott komponens: a **következmény-kapu emberi jóváhagyó felülete** — a `ConsequenceApprovalService.summarizeArgs` (a kártya és a modell-folytatás kivonata), a kártyát rajzoló `ticket-consequence-approvals.tsx`, valamint a hozzá tartozó mindig-kapuzott toolok arg-alakjai (`gmail_send`, `google_drive_share_file`, `google_drive_trash_file` — l. `consequence-gate-policy.ts` `ALWAYS_CONSEQUENCE_GATED_TOOLS`). Ez az a pont, ahol egy ember eldönti, kiengedhető-e egy visszafordíthatatlan vagy adatot kivivő agent-művelet; itt dől el, hogy a platform „az AI pontosan a kapott hozzáféréshez fér" ígérete a legkockázatosabb hívásoknál is érvényesül-e.
- Rendben talált kontrollok (a kapu maga erős): a `gmail_send` és a `google_drive_share_file`/`trash_file` mind mindig-kapuzottak; a jóváhagyás tenant- és szerep-kapuzott (idegen tenant a pending LÉTEZÉSÉT sem látja), az első jóváhagyás és a retry is egyszer-használatos (CAS `invoking`/`retrying` jelzőkkel), a `draftId`/mező-nélküli hibás hívást a registry fentebb elutasítja. Cross-tenant vagy kapu-megkerülési rést nem találtam.
- **Lelet (magas, kontroll-integritás / adatkiszivárgás — a HITL-kapu vakfoltja):** a jóváhagyó kártya kivonata egy általános, három-mezős heurisztikából (`path`/`to`/`title`) épült, ami épp a két legveszélyesebb, adat-kivivő eszköznél szinte semmit sem mutatott. **`google_drive_share_file`**: a kártya CSAK a tool nevét írta ki — se címzett, se jogszint, se fájl → a jóváhagyó vakon engedélyezhetett egy külső félnek szóló fájlmegosztást (ugyanaz az osztály, mint a memóriában jelzett „csendes külső Drive-szivárgás", #400). **`gmail_send`**: legfeljebb a címzettet mutatta; a tárgy és a szöveg soha nem látszott, a `draftId`-alapú küldésnél pedig SEMMI. Így egy tévedő vagy prompt-injektált agent ártalmatlannak tűnő címzettel, a törzsbe rejtett bizalmas adattal küldhetett levelet, és a jóváhagyó — csak a címzettet vagy a puszta tool-nevet látva — rábólintott. A kapu megvolt, de a döntéshez szükséges információ hiányzott: a védelmi réteg épp a legkockázatosabb hívásoknál nem tudott mit védeni.
- Javítás (a kaput a böngészőben tényleg megalapozott döntéssé teszi):
  - Új `describeConsequenceForHuman`: a nagy-következményű toolokra a kártya a TÉNYLEGES hatást írja ki. `gmail_send` → címzett + tárgy + a levélszöveg előnézete; `draftId` esetén KIMONDJA, hogy a tartalom itt nem látszik és a Gmail-piszkozatot külön kell ellenőrizni. `google_drive_share_file` → fájl + címzett + jogszint (olvasó/kommentelő/szerkesztő), csoport-címzett külön jelölve. `google_drive_trash_file` → melyik fájlt érinti.
  - A megjelenített értékek a jóváhagyásba MENTETT (a modell által, esetleg álnevesített formában előállított) args-ból jönnek — nyers titkot NEM oldunk fel a kártyához. A hossz-előnézet vágás megmarad; a modell-folytatás (external_untrusted boríték) és a dedup (args-ra kulcsol) viselkedése változatlan.
- A `/code-review` skill (Standards + Spec, párhuzamos sub-agentek) lefutott a diffen. Beépítve: (Spec) a többi mindig-kapuzott, még „vak" tool közül a Drive-kukázás is a fájlt mutatja; (Standards) nevesített `SUMMARY_ID_MAX_CHARS` konstans a 60/80 literálok helyett, `DRIVE_ROLE_LABELS` lookup a beágyazott ternár helyett, a visszatérési sztringek a `toolName` paramétert használják az újragépelt literál helyett. Nyitva hagyva (külön munka): a `draftId`-tartalom valódi feloldása a kártyára (Gmail-hívás), és a maradék mindig-kapuzott toolok (`sandbox.*`, `memory_propose`) gazdagabb kivonata.
- Üzleti hatás: egy enterprise AI platform legfőbb ígérete, hogy az AI pontosan a kapott hozzáféréshez fér — és hogy a kockázatos műveleteket ember látja és engedi. E hiba mellett az emberi jóváhagyás a legveszélyesebb (levél, fájlmegosztás) hívásoknál vakon történt: a jóváhagyó nem látta, mi és kinek megy ki. A javítás a döntéshez szükséges tartalmat a kártyára teszi, így a HITL-kapu ténylegesen betölti a kiszivárgást megfogó szerepét.
- Ellenőrzés: `node --import tsx scripts/consequence-approval.test.ts` zöld (a teljes suite + 4 új regresszió: gmail_send teljes mezőkkel; gmail_send draftId figyelmeztetéssel; Drive-megosztás címzett+jogszint; Drive-kukázás fájl-id). `npx tsc --noEmit` és célzott `eslint` tiszta a módosított fájlokon; `git diff --check` tiszta. (A `npm run test:*` a sandbox `tsx` IPC-EPERM miatt nem indul, ezért IPC-mentes `node --import tsx`.) Branch: `fix/consequence-card-outbound-content-visibility`, **PR #411 NYITVA**. (A review egy megosztott munkafában, párhuzamos `perf/neon-pool-connection-churn` session mellett készült; a javítás izolált git worktree-ben, a folyamatban lévő WIP érintése nélkül.)
