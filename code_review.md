...
## 2026-08-31 - Munkaterületi fájl-eszközök / XLSX-adapter: agent-kiváltható, több-tenantos futásidő-DoS a korlátlan A1-tartomány bejáráson

- Áttekintett, korábban külön nem naplózott komponens: a **fájl-szerkesztő XLSX-adapter** (`app/src/domain/file-editor/adapters/xlsx-adapter.ts`) és a köré épülő agent-facing eszközök (`file-editor-service.ts` → `tool-broker/handlers/file.handler.ts` → `tool-registry.ts` descriptorok). Ez az a réteg, ahol egy AI agent valódi Excel-munkafüzeteket olvas/ír/formáz a munkaterületen — a `file-editor-service.ts` path-traversal/ReDoS oldalát a 2026-07-25-i kör már nézte, de a **bináris dokumentum-adaptereket (xlsx/docx/pdf/pptx) és a formázó/elrendezés eszközöket** még nem. A `docx`/`pdf`/`pptx` adaptereken funkcionális/erőforrás-szempontból nem találtam blokkolót (a pdf-olvasás oldal-plafonos, a create-utak agent-adta adatból építkeznek); a lelet az XLSX formázó/elrendezés úton van.
- **Lelet (magas, availability / DoS):** az `xlsx_format_range` és az `xlsx_layout` agent-eszközök egy A1-tartomány **MINDEN celláját** bejárják (`for r … for c`, cellánként `getCell`), terület-plafon nélkül. A bemeneti Zod-séma csak a `range`-**string hosszát** korlátozza (max 40 karakter), a **területet nem** — így egy 13 karakteres `"A1:XFD1048576"` (16384×1 048 576 ≈ **17 milliárd cella**) gyakorlatilag végtelen ciklust + tömeges cella-allokációt indít. Mivel a Node-futásidő **megosztott, több-tenantos**, egyetlen ilyen hívás (akár véletlen rossz tartomány, akár **prompt-injektálás** egy nem-megbízható dokumentumból/weboldalból) az **összes tenant** szolgáltatását lefagyaszthatja/OOM-mal megölheti. A `parseA1Range` az Excel-korlátot sem nézte (pl. `ZZZ99999999` átment).
- **Lelet (magas, ugyanaz az eszköz — a `/code-review` Spec-tengelye találta):** az `xlsx_layout` **`mergeCells`** ága a tartomány-stringet közvetlenül az exceljs `worksheet.mergeCells(merge)`-nek adta át, **kikerülve a `parseA1Range` kaput**. Az exceljs a merge teljes területét cellánként bejárja (`_mergeCellsInternal`), így `mergeCells: ["A1:XFD1048576"]` (hívásonként max 200 elem) ugyanazt a DoS-t reprodukálta — a formázó út javítása önmagában nem fedte le.
- Javítás (a „egy agent egyetlen lépése ne dönthesse be a közös szolgáltatást" invariánst kényszeríti):
  - Közös terület-kapu a `parseA1Range`-ben (mindkét bejáró út egyetlen feloldási pontja), **a munkafüzet módosítása ELŐTT**: Excel-korlát (`XLSX_MAX_COLUMN=16384`, `XLSX_MAX_ROW=1 048 576`) + `XLSX_MAX_RANGE_CELLS=250 000` terület-plafon, tipizált `FileEditorError('INVALID_RANGE', …)` közérthető magyar üzenettel, ami a szűkebb tartomány / dedikált elrendezés-mezők felé terel.
  - A `mergeCells` ág is átmegy a közös kapun (`parseA1Range(merge)` a `mergeCells` hívás előtt).
- A `/code-review` skill (Standards + Spec, párhuzamos sub-agentek) lefutott a diffen. Standards: nincs kemény szabálysértés (magyar üzenetek, WHY-kommentek, üzleti-hatás keret rendben); az egyetlen megjegyzés a két kapu (Excel-korlát vs. terület) részleges átfedése — szándékos, mert külön, érthetőbb hibaüzenetet adnak. Spec: a fenti `mergeCells`-rést a review tárta fel, BEÉPÍTVE; a határeset-matek (XFD elfogadva/16385 elutasítva, plafon+1 elutasítva) és a mutáció-előtti hibázás igazolva.
- Ellenőrzés: `scripts/xlsx-range-and-delete-confirm.test.ts` (teljes-lap / plafon+1 / Excel-korláton túli elutasítva, valós tábla átmegy) és `scripts/xlsx-data-validation.test.ts` (teljes-lap mergeCells elutasítva, a lap nem módosul; kis merge átmegy) — mind zöld; `tsc --noEmit` az xlsx fájlokra tiszta.
- Munkakörnyezet-megjegyzés: a review közben a megosztott munkafát egy **párhuzamosan futó peer-session** (poll-endpoint-coalesce munka) többször átváltotta, sőt az azonos nevű feature-branchre commitolt is → a javítást **izolált `git worktree`-ben** fejeztem be, tiszta, main-alapú branchen (`fix/xlsx-range-dos-guard-clean`), hogy a PR ne keveredjen idegen WIP-pel. (Megerősíti a memóriabeli „mindig git-diff-eld a hátteret" tanulságot.)
- Üzleti hatás: egy enterprise AI platform rendelkezésre-állása alap-elvárás; egy Excel-formázó/-egyesítő kérés sosem áshatja alá az egész — több cégnek szolgáltató — rendszert. A javítás a felhasználó/agent felé érthető hibával elzárja ezt az utat, a valós táblaméretek érintése nélkül.
- Nyitva hagyva (külön feladat, nem e PR): a bináris adapterek (`xlsx/docx/pdf`) a nem-megbízható fájlt teljes egészében memóriába töltik; az 50 MB-os fájlkorlát a nyers méretet fogja, a tömörítés-arányt (zip-bomba) nem — dekompressziós-bomba elleni védelem külön vizsgálandó. (Branch: `fix/xlsx-range-dos-guard-clean`, **PR #405 NYITVA**.)
