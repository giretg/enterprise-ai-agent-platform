...
- Ellenőrzés: `test:privacy-category-policy` 15/15 zöld (2 új regressziós teszt az audit-tartalomra: `actions` + `rawEgressEnabled` + `policyDecision`, és a `null`→`inherit` naplózás). `tsc --noEmit` tiszta. A `/code-review` skill Standards+Spec tengelye lefutott. (Branch: `fix/privacy-pan-iban-egress-audit-and-stale-superadmin-gate`, PR: NYITÁS FOLYAMATBAN.)
