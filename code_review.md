...
## 2026-08-10 - Generikus http_api connector runtime: feloldás-utáni SSRF-őr hiánya a kezdeti kérésen (DNS-rebinding / belső-IP)

- Áttekintett, korábban külön nem naplózott komponens: a **generikus http_api connector runtime** — az az egress-motor, amit az agentek a `http_api_get` / `http_api_request` toolokkal a leggyakrabban hajtanak. Teljes egress-metszet: a kérés-összeállítás és a védett fetch (`app/src/domain/connector/http-api-client.ts` — `buildUrl`, `fetchWithBackoff`, `fetchFollowingSameOriginRedirects`), a közös SSRF-őr (`app/src/domain/net/egress-guard.ts` — `guardEgressUrl`, host-osztály minták, feloldás-utáni privát/reserved-IP re-check), a config-idejű draft-validáció (`app/src/domain/provisioning/draft-validator.ts`), és az egyetlen éles példányosító (`app/src/domain/tool-broker/tool-broker-delegation.ts`).
- Tenant-izolációs és redirect-SSRF értékelés: **erős**. A path nem írhatja felül a hostot (`selectEndpoint` elutasít minden `://`-t; a baseUrl authority fixálja a hostot), a `fetch` `redirect: 'manual'`, és a 3xx-eket a kliens a connector SAJÁT originjére (séma+host+port) pinnelve követi — az idegen/metadata/port-váltó/`https→http` redirect zárva (a korábbi #214/#238 munkák). Ezen nem találtam rést.
- **Lelet (magas, SSRF / belső-hálózat elérés):** a feloldás-utáni privát/reserved-IP re-check (a `guardEgressUrl` `resolveHostIps` ága, DNS-rebinding elleni védelem) CSAK a `selfUpdatingPinned` (önfrissítő) connectoroknál futott. A normál connectorok — az agentek fő egress-útja — a konfigurált `baseUrl` hostjára minden futásidejű SSRF re-check nélkül lőttek ki. A `baseUrl` hostot létrehozáskor csak statikus mintaszűrés nézi (nyers privát-IP host, pl. `10.0.0.5` → csak `warned`), DNS-feloldás nélkül. Így két úton is elérhetővé vált a belső hálózat: (1) egy tenant-admin belső-IP-re állított `baseUrl`-je, és (2) DNS-rebinding — egy létrehozáskor publikus, érvényes hostnév, ami futásidőben belső/metadata IP-re old fel. Mindkét esetben az agent a `path`/`query` felett rendelkezik, és a válasz body visszakerül a modellhez → vak SSRF + belső adatszivárgás.
- Javítás:
  - A nem-pin-elt connectorok KEZDETI kérése is átmegy egy futásidejű egress-őrön (`assertRuntimeEgressAllowed`): (a) mindig lefutó, hálózat nélküli host-minta szűrő (nyers privát/loopback IP-literál, localhost, felhő-metadata host, ismert exfil-sink), és (b) bekötött DNS-feloldó esetén feloldás-utáni privát/reserved-IP re-check — deny-by-default, a fetch el sem indul.
  - A DNS-feloldó a Tool Brokerben (az egyetlen éles hívási hely) a valós `node:dns`-re van kötve (fail-closed alapérték), és injektálható a determinisztikus, hálózat nélküli tesztekhez.
  - A feloldás-utáni IP re-check MAGJA és a `detail`-kódjai egyetlen közös helperbe (`checkResolvedHostIps`) kerültek, amit a `guardEgressUrl` és a connector-runtime is hív — így a tiltott osztályok és a hibakódok egy helyen élnek, nem csúsznak el (a `/code-review` Standards-tengely „Duplicated Code” leletének feloldása). Az élő feloldó egyetlen közös `nodeDnsResolveHostIps`.
  - A `guardEgressUrl` és a pin-elt ág viselkedése (séma-kényszer, allowlist, reason-kódok, sorrend) bitre változatlan; a `http://` baseUrl-t szándékosan NEM utasítjuk el (a `guardEgressUrl` teljes újrahasználata törte volna a legitim http connectorokat).
- Üzleti hatás: a platform fő külső-API csatornája ugyanarra a deny-by-default egress-szintre került, mint a `web_fetch` és az önfrissítő connectorok. Egy agent (vagy egy kompromittált/hibásan konfigurált tenant-connector) nem érheti el a felhő-metadata végpontot, a belső admin-felületeket vagy a privát hálózatot egy connector-híváson keresztül — sem közvetlen belső-IP hoszttal, sem DNS-rebindinggel. Ez zárja az egyik legmagasabb kockázatú, agent-vezérelt egress-rést egy éles enterprise telepítés előtt.
- Ellenőrzés: `npm run test:http-api` (4 új eset: nyers privát-IP baseUrl → blokk; metadata baseUrl → blokk; DNS-rebinding publikus→privát → blokk; publikus IP → átmegy), `npm run test:http-api-per-agent-key` (injektált feloldóval, hálózat nélkül), `scripts/egress-guard.test.ts` (a refaktor után is zöld, incl. EG-11/11b/12 DNS-rebinding), célzott `tsc --noEmit` tiszta. A `/code-review` skill Standards+Spec tengelye lefutott: Spec átment, a Standards „Duplicated Code” leletet a közös helper feloldotta. (PR: `pr/connector-runtime-egress-ssrf-guard`.)
